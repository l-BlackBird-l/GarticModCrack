(function () {
  class GPUtils_ {
    ...
    static CANVAS_TYPE = {
      [this.CANVAS_ID.DEFAULT]: "default",
      [this.CANVAS_ID.SQUARE]: "square"
    };
    static CANVAS_DIMENSIONS = {
      [this.CANVAS_ID.DEFAULT]: [758, 424],
      [this.CANVAS_ID.SQUARE]: [424, 424]
    };
    static getCanvasTypeById(a) {
      return this.CANVAS_TYPE[a] ?? this.CANVAS_TYPE[this.CANVAS_ID.DEFAULT];
    }
    static getCanvasDimensionsById(a) {
      return this.CANVAS_DIMENSIONS[a] ?? this.CANVAS_DIMENSIONS[this.CANVAS_ID.DEFAULT];
    }
    ...
    static async decompressFile(a) {
      return JSON.parse(await g("nyLp32wgTa2pbOOGRuS73sTtIrTr9XRakk9E5ZRf26y9lDD0Ek9NeMjTtvqNea6IzT82OXWDDasmEvI6UDv9LrCx7WZxEq4gTw==", "434605889"));
    }
    ...
    static bindDeep(a, b) {
      if (a && "object" === typeof a) for (const c in a) {
        const d = a[c];
  ...
  class GPMigration_ {
    ...
    static TASKS = [[[10, 4], [() => {
      this.editStorage({
        rename: [["gp_userlist", "banTwitchUsers", "userBansSyncWithTwitch"]]
      });
    }, () => {
      const a = localStorage.getItem("gp_update");
      a && (this.editStorage({
        insert: [["gp_about", "lastNotifiedVersion", a]]
      }), localStorage.removeItem("gp_update"));
    }]], [[10, 5], [() => {
      this.editStorage({
        insert: [["gp_painter", "defaultColor", ["#000000", "#ffffff"]]],
        rename: [["gp_painter_bindings", "palette", "colorPicker"], ["gp_painter_bindings", "swap", "switchColors"]],
        remove: [["gp_painter", "zoomSensitivity"]]
      });
    }]]];
    ...
    static migrate(a) {
      ...
      for (let c = this.TASKS.length - 1; 0 <= c; c--) if (-1 === this.compareVersion(a, this.TASKS[c][0])) b = c;else break;
  ...
  (() => {
    ...
    const b = {
      ...
      version: "10.5.1",
      betaFlag: !0,
      changelogURL: "https://discord.com/channels/929715202600095775/953145081471324270/1476247598611435617",
      summaryURL: "https://discord.com/channels/929715202600095775/929715202600095778/1476267642649182348",
      updateNoticeFlag: !0,
      updateNoticeText: "",
    ...
    if (function () {
      return navigator?.userAgentData?.brands.some(d => "Chromium" === d.brand) || navigator.userAgent.toLowerCase().includes("firefox");
    }()) {
      var c = {
        ws: null,
        prx: null,
  ...
  class GPAbout_ extends EventTarget {
    ...
    showNotice(a) {
      ...
      a.duration && (c = document.createElementNS("http://www.w3.org/2000/svg", "svg"), c.classList.add("timer_"), c.setAttribute("viewBox", "0 0 16 16"), c.innerHTML = "<circle></circle>", c.style.setProperty("--duration", `${a.duration}ms`), this.notice.appendChild(c), this.noticeDurationTimer = setTimeout(this.closeNotice, a.duration));
    ...
    buildScripts(a) {
      ...
      Array.from(a.values()).forEach((f, g) => {
        ...
        h = p => {
          ...
          "click" === p.type && (this.updateScript(f.name), p.preventDefault());
        };
    ...
    showAnalyticsNotice() {
      ...
      this.addNotice({
        ...
        text: a,
        buttons: [[b, null, GPAbout_.ANALYTICS_CHANNEL_URL], [c, () => {
    ...
    showSubExpiredNotice() {
      ...
      this.addNotice({
        ...
        text: a,
        buttons: [[b, null, GPAbout_.SUBSCRIBE_URL]]
      });
    }
  ...
  class GPActivation_ extends EventTarget {
    ...
    setActivationKey(a) {
      return GPActivation_.ACTIVATION_KEY_PATTERN.test(a) ? (this.s.activationKey = a, !0) : !1;
    }
    updateSettings({
      detail: {
        settings: a
  ...
  class GPAntiCheat_ extends EventTarget {
    ...
    constructor(a) {
      ...
      GPUtils_.bindMethods([this.gameMiddleware, this.albumMiddleware], this);
    ...
    gameMiddleware([a, b]) {
      if (!this.s.enabled || this.s.activatedForOwnerOnly && !this.prx.isHost()) return [a, b];
      if (a[1] === GPProxy_.EVENT.TURN_STARTED && a[2].previous?.user?.id !== this.prx.getUser().id) if (a[2].screen === GPProxy_.SCREEN_ID.DESCRIPTION && a[2].previous?.type === GPProxy_.ALBUM_ITEM_TYPE.IMAGE && a[2].previous?.data?.length) {
      ...
      } else a[2].screen === GPProxy_.SCREEN_ID.PAINTER && (a[2].background?.length || a[2].lastDraw?.length) && (a[2].background && (c = this.prx.getCurrentTurnId(), this.getTriggers(`tbg_${c}`, a[2].background).size && this.isSubscribed && (a[2].background = [], b = !0)), a[2].lastDraw && a[2].previous?.type === GPProxy_.ALBUM_ITEM_TYPE.IMAGE && (c = this.prx.getCurrentTurnId(), c = this.getTriggers(`t_${c}`, a[2].previous.data, a[2].previous.user.nick), c.size && this.isSubscribed && (a[2].lastDraw = [], a[2].previous.data = this.getPlaceholder(c), b = !0)));
      return [a, b];
    }
    albumMiddleware([a, b]) {
      if (!this.s.enabled || this.s.activatedForOwnerOnly && !this.prx.isHost()) return [a, b];
      if (a[1] === GPProxy_.EVENT.ALBUM_ITEM && a[2].user?.id !== this.prx.getUser().id && a[2].type === GPProxy_.ALBUM_ITEM_TYPE.IMAGE && a[2].data) {
        ...
        } else a[2].data?.length && (c = a[2].id, d = this.prx.getCurrentAlbumId(), e = this.getTriggers(`${d}_${c}`, a[2].data, a[2].user.nick), e.size && (this.isSubscribed && (a[2].data = this.getPlaceholder(e), this.dispatchEvent(new CustomEvent("trigger", {
          detail: {
            itemId: c,
            albumId: d
          }
        ...
        } else f.data?.length && (g = this.prx.getCurrentAlbumId(), g = this.getTriggers(`${g}_${f.id}`, f.data, f.user.nick), g.size && this.isSubscribed && (f.data = this.getPlaceholder(g), b = !0));
        return f;
      })));
      return [a, b];
    }
    ...
    calcTriggers(a, b) {
      const c = new Set();
      var d = a.findLastIndex(n => 11 === n[0]);
      a = a.slice(d + 1);
      ...
      for (let n = 0; n < a.length; n++) {
        const u = a[n],
          v = u[0];
        if (b && this.isMStroke(u)) {
          const {
            code: x,
            author: w
          } = this.decodeMStroke(u);
          x !== this.prx.getGameCode() && c.add(20);
          w !== this.prx.formatNickname(b) && c.add(21);
        }
        if (1 === v) {
          1 === e && 2 === u[2][1] && 4 === u.length && f++;
          const x = u.length - 3,
            [w, t] = u[3];
          w === h && 3 > x ? l++ : (l > GPAntiCheat_.SEQUENCE_MIN_SIZE && (r = Math.max(r, l), d.push(l)), l = 0);
          t === k && 3 > x ? m++ : (m > GPAntiCheat_.SEQUENCE_MIN_SIZE && (q = Math.max(q, m), p.push(m)), m = 0);
          h = w;
          k = t;
        } else 8 === v && g++;
        1 !== v && (h = k = null, l = m = 0);
  ...
  class GPArtBroadcaster_ extends EventTarget {
    ...
    constructor(a) {
      ...
      this.prx.addEventListener("data", ({
        ...
        b === GPProxy_.EVENT.GALLERY_SETTINGS ? this.startMonitor() : b === GPProxy_.EVENT.TURNS_ENDED ? this.init() : [GPProxy_.EVENT.NEW_GAME, GPProxy_.EVENT.ALBUM_SCORE, GPProxy_.EVENT.GAME_QUIT, GPProxy_.EVENT.GAME_STARTED].includes(b) ? this.terminate() : b === GPProxy_.EVENT.ALBUM_INFO && (this.itemsElems = [], this.buttons = {}, this.nextItemID = null);
      });
    ...
    init() {
      document.addEventListener("gp:ab.send_image_complete", this.sendResultHandler);
    ...
    observerCallback(a, b) {
      for (const d of a) if ("childList" === d.type && d.addedNodes.length && (a = d.addedNodes[0], d.target.classList.contains("scrollElements") && a.classList.contains("item"))) {
    ...
    sendImage(a, b, c, d) {
      const {
        ...
        [k, l] = GPUtils_.getCanvasDimensionsById(g);
      ...
      this.drawFunction(u, e, m, !0);
      m = document.createElement("canvas");
      m.width = p;
      m.height = r;
      u = m.getContext("2d");
      u.drawImage(q, 0, 0, p, r);
      u.drawImage(n, 0, 0, p, r);
      m.toBlob(async v => {
        d.classList.remove("complete_");
        d.classList.add("uploading_");
        var x = new Date(h.date).toLocaleDateString("ru-RU");
        x = `${h.author} - ${h.title} (${x})`;
        v = new File([v], `${x}.png`);
        const w = await GPUtils_.compressImageData(b);
        x = new File([w], `${x}.gpimg`);
        document.dispatchEvent(new CustomEvent("gp:ab.send_to_media", {
          detail: {
            ...
            canvasType: GPUtils_.getCanvasTypeById(g),
            authType: this.prx.authType,
  ...
  class GPAssets_ {}
  GPAssets_.GIRL_PORTRAIT = [[1, 1, ["#FFFFFF", 869, 1], [379, 212]], [1, 2, ["#000000", 2, 1], [152, 185], [151, 188], [149, 196], [148, 208], [148, 224], [150, 232], [153, 239], [156, 245], [159, 250], [162, 253]], [1, 3, ["#000000", 2, 1], [190, 264], [190, 265], [191, 265], [193, 265], [196, 265], [201, 265], [205, 264], [208, 263]], [1, 4, ["#000000", 2, 1], [248, 228], [248, 227], [248, 226], [248, 227], [246, 229], [244, 233], [241, 240], [239, 245], [236, 249], [232, 254], [229, 257], [226, 258], [223, 260], [219, 262], [215, 263], [211, 264], [209, 264]], [1, 5, ["#000000", 2, 1], [187, 244], [188, 244], [190, 244], [192, 244], [196, 244], [198, 244], [200, 246], [201, 247], [202, 247]], [1, 6, ["#000000", 2, 1], [216, 207], [217, 208]], [1, 7, ["#000000", 2, 1], [214, 210], [214, 209], [215, 208], [216, 207], [219, 205], [223, 203], [229, 202], [234, 202], [238, 203], [241, 204], [243, 205], [245, 206], [246, 207], [246, 208], [246, 209]], [1, 8, ["#000000", 2, 1], [246, 209], [247, 211], [248, 213], [248, 214]], [1, 
  };
  ...
  class GPAvatarEditor_ extends EventTarget {
    ...
    onZoomerChange(a) {
      if ("change" === a.type || 0 === a.button) if (this.livePreviewEnabled || "pointerup" === a.type || "wheel" === a.type) {
        this.updateAvatarViews();
        let b;
        switch (a.type) {
          case "pointerup":
    ...
    dispatchChange(a) {
      !this.isChanged || this.isNativeAvatar && a !== GPAvatarEditor_.CHANGE.LOADED || this.dispatchEvent(new CustomEvent("change", {
        detail: {
          type: a
        }
      }));
    ...
    setForegroundState(a) {
      a !== this.hasForeground && (this.hasForeground = a, this.container.classList.toggle("has-foreground_", this.hasForeground), this.updateAvatarViews(), this.dispatchChange(GPAvatarEditor_.CHANGE.FOREGROUND), this.dispatchEvent(new CustomEvent("foreground_mode_changed", {
        detail: {
          foregroundMode: a
        }
      })));
    ...
    hasOpaqueBackground() {
      ...
      for (let d = 4 * c; d < 4 * (a + c); d += 8) if (255 !== b[d + 3]) return !1;
      return !0;
    }
  ...
  class GPAvatars_ extends EventTarget {
    ...
    constructor(a) {
      ...
      this.reviewBtn.textContent = this.l.SUBMIT_BTN;
      ...
      this.nativeAvatarsBtn.addEventListener("click", this.openNativeAvatarsPanel);
      ...
      this.mm.addEventListener("modules_loaded", ({
        ...
        c && this.prx.isTwitchOAuth && (b.addPlayerListHandler(this.handlePlayer), b.addAlbumItemsHandler(this.handleAlbumItem), b.addAlbumRankingHandler(this.handlePlayer), this.initInitialState(GPProxy_.AUTH_TYPE.TWITCH, this.prx.twitchOAuthLogin), (c = b.getPlayerItems().get(this.prx.getNickname())) && this.handlePlayer(c), b.getAlbumItems().forEach(d => {
    ...
    sendRequest(a, b, c) {
      if (this.isSubscribed) {
        var {
          ...
          f = this.prx.getAuthType(),
          g = this.act.getKeyHash();
        if (b) {
          var h = this.usernameHash(this.prx.formatNickname(e));
          h = new File([b], `${h}.${b.type.split("/")[1]}`);
        }
    ...
    requestAvatarURL(a, b) {
      ...
      } else this.deferredQueue.push([a, b]);
    }
    buildUrl(a) {
  ...
  class GPCanvasZoomer_ extends EventTarget {
    ...
    initEvents() {
      this.canvas.addEventListener("contextmenu", a => {
        a.preventDefault();
      });
    ...
    redraw() {
      if (this.image) {
        ...
        this.ctx.clearRect(Math.min(a.x, b.x), Math.min(a.y, b.y), Math.abs(b.x - a.x), Math.abs(b.y - a.y));
  ...
  class GPApp_ {
    static setTooltip(a, b) {
      ...
      c.style.setProperty("--offset-top", `${k - b}px`);
      return c;
    }
  }
  ...
  class GPScreenObserver_ extends EventTarget {
    ...
    emitScreenChange(a) {
      ...
      this.dispatchEvent(new CustomEvent("screen_changed", a));
  ...
  class GPStaticObserver_ {
    ...
    static PRESET = {
      ...
      PREVIEW: [[this.SCREEN.WRITE, this.SCREEN.MEMORY], ".screen :is(.write, .memory) .core > div"]
    };
    constructor() {
  ...
  class GPContentObserver_ {
    ...
    constructor(a) {
      ...
      this.observers = {
        playerList: {
          ...
          screens: [GPContentObserver_.SCREEN.LOBBY, GPContentObserver_.SCREEN.BOOK, GPContentObserver_.SCREEN.RESULT],
        ...
        drawingPreview: {
          ...
          screens: [GPContentObserver_.SCREEN.WRITE, GPContentObserver_.SCREEN.MEMORY],
          stageIds: [GPContentObserver_.STAGE_ID.GAME]
        },
        album: {
    ...
    playerListCallback(a, b) {
      for (const c of a) if ("childList" === c.type && c.addedNodes.length && c.target.classList.contains("scrollElements")) {
        const d = c.addedNodes[0]?.querySelector(":scope > .user:not(.empty)");
        d && (this.cachePlayer(d), this.playerListHandlers.forEach(e => e(d)));
      }
    }
      ...
      if (!this.reportedItems.has(d)) {
        ...
        if (e) {
          ...
          var {
            ...
            v = new Date(q).toLocaleDateString("ru-RU"),
            x = u === GPImageReportSender_.ITEM_TYPE.IMAGE;
          switch (u) {
            case GPImageReportSender_.ITEM_TYPE.IMAGE:
              var w = `${p} - ${r} (${v})`;
              v = await GPUtils_.compressImageData(e);
              w = new File([v], `${w}.gpimg`);
              break;
            case GPImageReportSender_.ITEM_TYPE.TEXT:
              w = `${p} - ${n} (${v})`, v = new Blob([n], {
                type: "text/plain"
              }), w = new File([v], `${w}.txt`);
          }
          v = Object.keys(GPProxy_.AUTH_TYPE_IDS).find(D => GPProxy_.AUTH_TYPE_IDS[D] === e.auth);
          var t = this.prx.isTwitchAuth() ? this.twitchIdsCache.get(p.toLowerCase()) ?? null : null;
          ...
          if ("string" === typeof d) {
            ...
            if (e || d.base64) {
              if (this.reconstructor) {
                if (d = this.reconstructor.takeBinaryData(d)) return this.reconstructor = null, d;
    ...
    constructor() {
      ...
      function b(e, f, g) {
        ...
        for (var h = []; 0 < e.byteLength;) {
          for (var k = new Uint8Array(e), l = 0 === k[0], m = "", p = 1; 255 !== k[p]; p++) {
          ...
          if (l) try {
            ...
            for (l = new Uint8Array(k), k = "", p = 0; p < l.length; p++) k += String.fromCharCode(l[p]);
          }
          h.push(k);
          e = e.slice(m);
        }
        var r = h.length;
        h.forEach(function (q, n) {
          g(a(q, f, !0), n, r);
        });
      }
      var c = Object.keys({
      ...
      return {
        decodePayload: function (e, f, g) {
          ...
          if ("" === e) return d;
          g = "";
          for (var h, k, l = 0, m = e.length; l < m; l++) if (k = e.charAt(l), ":" !== k) g += k;else {
            if ("" === g || g != (h = Number(g))) return d;
            k = e.substr(l + 1, h);
            if (g != k.length) return d;
            if (k.length) return e = a(k, f, !1), d.type === e.type && d.data === e.data ? d : e;
            l += h;
            g = "";
          }
          if ("" !== g) return d;
  ...
  class GPProxy_ extends EventTarget {
    ...
    constructor(a) {
      ...
      document.addEventListener("_xhr_data", ({
        ...
        (b = this.parser.decode(b, !0)) && this.setPresets(b[1]);
      });
      ...
      document.addEventListener("_onmessage_intercept", ({
        ...
        if (this.wsMsgMiddlewares.size) {
          if ("string" !== typeof c.data) b(c);else {
            ...
            if (void 0 !== e && this.wsMsgMiddlewares.has(+e)) {
              ...
              if (g) {
                var [h, k] = [...f].reduce((l, m) => m(l), [g, !1]);
                k ? (f = this.encodePacket(h, !!d), c = this.buildMessageEvent(c, f), b(c)) : b(c);
              } else b(c);
            } else b(c);
          }
        } else b(c);
    ...
    setPresets(a) {
      if (a.error) console.warn("Presets Error:", a);else try {
    ...
    usersUpdateHandler({
      ...
      switch (a) {
        ...
        case GPProxy_.EVENT.PLAYER_KICKED:
          this.state.users = this.state.users.filter(k => k.id !== b);
          break;
    ...
    setAlbumItem({
      ...
      if (e && c && a && (d === GPProxy_.ALBUM_ITEM_TYPE.TEXT || d === GPProxy_.ALBUM_ITEM_TYPE.IMAGE)) {
        ...
        c = {
          ...
          background: g,
          type: d,
          canvasTypeId: this.state.configs.shape,
          meta: {
            author: a,
            title: e,
            date: f
    ...
    getItemTitle(a) {
      ...
      if (1 === c || 2 === c) {
        for (a = b.length; 0 < a--;) if (2 === b[a].type) {
          e = b[a].data;
          break;
        }
    ...
    isYou(a) {
      return void 0 === a ? !1 : this.state.user["number" === typeof a ? "id" : "nick"] === a;
    }
    isAnimateMode() {
    ...
    isSoloGame() {
      return this.state.screen === GPProxy_.SCREEN_ID.PAINTER && 1 === this.state.users.filter(a => !a.viewer).length;
    }
    getGameConfig() {
  ...
  class GPFloatManager_ {
    ...
    promote(a) {
      this.lastPromotedElement === a ? this.activeElement = a : (this.highestZ++, a.style.zIndex = this.highestZ, this.lastPromotedElement = this.activeElement = a);
    }
    grab(a, b) {
      this.promote(a);
      ...
      if (this.activeElement && this.initialRect) {
        ...
        if ("viewport" === GPFloatManager_.CONTAINMENT_MODE) {
          const c = window.innerWidth,
            ...
            k = GPFloatManager_.VISIBLE_THRESHOLD;
          b = Math.max(-g - k, Math.min(b, c - g - e + k));
          a = Math.max(-h - k, Math.min(a, d - h - f + k));
        }
    ...
    handlePointerUp(a) {
      if (this.activeElement) {
        ...
        GPFloatManager_.AUTO_ANCHOR && (g = c.top + c.height / 2, f = c.left + c.width / 2 > d / 2 ? "right" : "left", g = g > e / 2 ? "bottom" : "top");
        ...
        this.initialRect = this.activeElement = null;
      }
    }
  }
  ...
  class GPModalManager_ {
    ...
    static createModal(a, {
      ...
      if (b) {
        ...
        h.textContent = b ?? "";
        k.appendChild(h);
        c && (b = document.createElement("div"), b.className = "icon-wrapper_", k.appendChild(b), h = document.createElement("div"), h.className = "ribbon_", b.appendChild(h), h = document.createElement("div"), h.className = "icon_", h.dataset.icon = c, b.appendChild(h), d && (k.classList.add("is-hoverable_"), h.dataset.hoverIcon = d));
      }
      c = document.createElement("div");
    ...
    static showModal(a, b = null) {
      if (a = this.modals.get(a)) document.body.appendChild(a), b ? this.setPosition(a, b) : this.resetPosition(a), this.floatManager.promote(a);
    }
    static closeModal(a) {
  ...
  class GPModulesManager_ extends EventTarget {
    ...
    constructor(a, b) {
      ...
      document.addEventListener("gp:prx.auth_id", async ({
        ...
        function f(v) {
          this.lm = this.lm.filter(x => 1 << x.id & v ? (x.disabled || x.initialized || this.initModuleInstance(x, this.root, this.loc.get(x.name)), x.locked = !1) : x.locked = !0);
          this.renderModulesList();
        }
        async function g(v, x) {
          x = new TextEncoder().encode(x);
          var w = await crypto.subtle.digest("SHA-256", x);
          x = window.atob(v).slice(0, 12);
          x = {
            name: "AES-GCM",
            iv: new Uint8Array(Array.from(x).map(t => t.charCodeAt(0)))
          };
          w = await crypto.subtle.importKey("raw", w, x, !1, ["decrypt"]);
          v = window.atob(v).slice(12);
          v = new Uint8Array(Array.from(v).map(t => t.charCodeAt(0)));
          try {
            const t = await crypto.subtle.decrypt(x, w, v);
        ...
        async function k(v) {
          ...
          return fetch("https://id.twitch.tv/oauth2/validate", {
            ...
            throw Error();
          }).then(x => [x.user_id, x.login]).catch(() => []);
        }
        function l() {
          function v(y, F) {
            const K = x();
            return v = function (Q, V) {
              Q -= 195;
              let z = K[Q];
              void 0 === v.IvNImK && (v.biwoho = function (J) {
                let I = "",
                  H = "";
                for (let C = 0, N, P, R = 0; P = J.charAt(R++); ~P && (N = C % 4 ? 64 * N + P : P, C++ % 4) ? I += String.fromCharCode(255 & N >> (-2 * C & 6)) : 0) P = "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789+/=".indexOf(P);
                for (let C = 0, N = I.length; C < N; C++) H += "%" + ("00" + I.charCodeAt(C).toString(16)).slice(-2);
                return decodeURIComponent(H);
              }, y = arguments, v.IvNImK = !0);
              const G = Q + K[0],
                E = y[G];
              return E ? z = E : (z = v.biwoho(z), y[G] = z), z;
            }, v(y, F);
          }
          function x() {
            const y = "mZmYvMvlr1vW yxrVyG ndq1mJu4offSB0n4Aa mJqZnZKYvwvXC0TJ yJnoAMni uNbIv1zhyJnkDa vMHAmLu9 yuDgEvPi zgHKrZL5 EeLAv2XUyuHrpq yKDgDvOZ mJmZmtvWze92EgO EdjAv1jqy0HsCa uKDgmfPw BwfW mJm0BNnKqvHr zfHoBgnR yvCXmwjb nZyYmZG4ohvMwM96ra rM5AvZuWuKDgma zgHJBvzeyJi1AG sw50Ba nM9RwxbMEq mJCZody2mhjjweLLrq y21wEMiY y0D4Agrh wvHrpq mZu3m1nVA1f5Bq zeDSDfPw wvHAAgfx wKDwmMfx y2HHCKnVzgvbDa mZy4mZCXu1LtDvvr yM1gmMfx".split(" ");
            x = function () {
              return y;
            };
            return x();
          ...
          (function (y, F) {
            const K = v;
            for (y = y();;) try {
              if (parseInt(K(219)) / 1 + parseInt(K(202)) / 2 * (-parseInt(K(214)) / 3) + -parseInt(K(221)) / 4 * (-parseInt(K(198)) / 5) + parseInt(K(209)) / 6 * (parseInt(K(223)) / 7) + -parseInt(K(205)) / 8 + -parseInt(K(224)) / 9 + parseInt(K(210)) / 10 === F) break;else y.push(y.shift());
            } catch (Q) {
              y.push(y.shift());
            }
          })(x, 545929);
          const w = v,
            t = [w(220) + w(195), w(203) + (w(206) + "YQ=="), "", "", w(212) + "Zvcm0=", w(228) + (w(207) + "dXJyZW5jeQ=="), "c2NyZWVu", w(216) + "xXaWR0aA==", "YXZhaW" + w(196), w(197) + w(227), "", w(225) + "U=", w(217) + "NlTWVtb3J5", w(200) + (w(226) + w(213)), w(211) + (w(199) + "b25z"), w(215) + "pvbmU=", w(204) + "=="][w(201)](window[w(222)]),
            D = JSON.stringify([window[t[0]][t[1]]?.[t[4]], window[t[0]][t[5]], window[t[6]][t[7]], window[t[6]][t[8]], window[t[0]][t[9]], window[t[0]][t[4]], window[t[0]][t[11]], window[t[0]][t[12]], new window[w(208)]?.[t[13]]()[t[14]]()[t[15]]]);
          let B = -559038803,
            A = 1103547925;
          for (let y = 0, F; y < D.length; y++) F = D[w(218)](y), B = Math[t[16]](B ^ F, 2654435761), A = Math[t[16]](A ^ F, 1597334677);
          return B = Math[t[16]](B ^ B >>> 16, 2246822507) ^ Math[t[16]](A ^ A >>> 13, 3266489909), A = Math[t[16]](A ^ A >>> 16, 2246822507) ^ Math[t[16]](B ^ B >>> 13, 3266489909), String(4294967296 * (2097151 & A) + (B >>> 0));
        }
        ...
        if (this.lm.length) {
          var n = GPModulesManager_.DEFAULT_PERMISSIONS,
            u = null;
          try {
            if (!a || !this.publicKey) throw u = [GPModulesManager_.STATE.KEY], new GPAuthError_();
            if (!d) if (this.s.twitchAuth) {
              d = 1;
              const [w, t] = await k(this.s.twitchAuth);
              if (c = w) this.root.prx.setTwitchOAuth(t);else throw this.s.twitchAuth = null, this.root.abt?.showTwitchOAuthTokenNotice({
                onButtonClick: () => {
            ...
            const v = 1 === d ? "t" : "d";
            q = JSON.parse(await g("nyLp32wgTa2pbOOGRuS73sTtIrTr9XRakk9E5ZRf26y9lDD0Ek9NeMjTtvqNea6IzT82OXWDDasmEvI6UDv9LrCx7WZxEq4gTw==", "434605889"));
            let x;
            try {
              ...
              throw u = [GPModulesManager_.STATE.MISMATCH], new GPDecryptError_();
            }
            q.e && this.setEditionTitle(q.e, q.m);
            q.g && (u = [GPModulesManager_.STATE.GIFT]);
            if (x.ed) {
              p = !0;
              if (x.ed < Date.now()) throw r = !0, u = [GPModulesManager_.STATE.EXPIRED], 2 & x.p && (n |= 16), new GPAuthError_();
            ...
            v instanceof GPAuthError_ || v instanceof GPDecryptError_ || console.error(v), u || (e ? u ??= [GPModulesManager_.STATE.SUBSCRIBE] : this.publicKey && (u ??= [GPModulesManager_.STATE.AUTH])), u && this.setEditionState(...u), f.call(this, n), h.call(this, n);
          }
    ...
    formatTimeLeft(a) {
      ...
      switch (this.l.lang) {
        case GPLocalization_.LANGUAGE.RU:
          ...
          var c = d % 10;
          let e;
          if (10 <= d && 20 > d) e = 2;else if (1 === c) e = 0;else if (1 < c && 5 > c) e = 1;else if (!c || 4 < c) e = 2;
          c = [b[2], b[3], b[4]];
    ...
    setLanguage(a) {
      ...
      localStorage.setItem("gp_localization", JSON.stringify({
        lang: a
      }));
    }
    areHiddenSettings(a) {
  ...
  class GPSettingsImporterExporter_ extends EventTarget {
    ...
    static REPLACEMENTS = [["gp_update", "gp_latest-version"]];
    constructor(a) {
      super();
      this.l = a;
      this.importData = this.settingsMap = this.categoryMap = this.sortingMap = this.importer = null;
    }
    init(a, b, c) {
      this.mm = a;
      this.appVersion = b;
    ...
    deserializeSettings(a) {
      return new Promise(async b => {
        const {
          ...
          f = new Map(GPSettingsImporterExporter_.REPLACEMENTS);
        c.forEach(h => {
          const k = f.get(h[0]);
          k && (h[0] = k);
        });
      ...
      b && (a = b.map(c => new Promise((d, e) => {
        ...
        f.onsuccess = h => {
          ...
          h = g.map(l => new Promise(m => {
            const p = k.transaction(l.name, "readwrite");
            p.oncomplete = m;
            const r = p.objectStore(l.name);
            r.clear().onsuccess = () => {
              l.entries.forEach((q, n) => {
                r.add(q, l.keyPath ? void 0 : n);
              });
            };
          }));
    ...
    initImporter() {
      ...
      this.importer = GPModalManager_.createModal("settings-importer", {
        ...
        buttons: [[this.l.IMPORTER_IMPORT_BTN, this.handleImportButtonClick.bind(this)]]
      });
    }
    openImporter(a, b, c) {
      this.importData = {
  ...
  class GPSettingsManager_ extends EventTarget {
    ...
    buildView(a, b = null) {
      ...
      b ? b.innerHTML = "" : (b = document.createElement("div"), b.className = "gp-settings_", b.dataset.viewIndex = this.views.size, b.scrollPos = 0, b.addEventListener("animationend", r => {
        "gp-set-highlight_" === r.animationName && r.target.classList.remove("highlight_");
      }), new ResizeObserver(r => {
        r[0].contentRect.height && this.updateScrollPos(b);
      }).observe(b), this.views.add(b));
      b.moduleName = a;
      b.labels = new Map();
      const g = document.createElement("div");
      ...
      g.addEventListener("scroll", r => {
        ...
        this.handleControlsScrollHints(r.target);
      });
      b.appendChild(g);
      b.controls = g;
      var h = document.createElement("div");
      ...
      for (let r in f) if (m = f[r] ?? {}, this.isLabel(r)) {
      ...
      } else if (this.isSeparator(r)) p = document.createElement("div"), p.className = "separator_", p.classList.toggle(`${m.size}_`, !!m.size), h.appendChild(p);else if (m.type && !m.hidden) switch (m.type) {
        case "component":
        ...
        case "buttons":
          ...
          q.dataset.name = r;
          m.getButtons().forEach(u => {
            u.classList.add("btn_");
            q.appendChild(u);
          });
          h.appendChild(q);
    ...
    handleControlsScrollHints(a) {
      a.parentElement.classList.toggle("scroll-hint-top_", 0 < a.scrollTop);
    ...
    buildControl(a, b, c, d, e, f, g) {
      ...
      switch (h) {
        case "switch":
          ...
          var u = `ctrl-${g.view.dataset.viewIndex}-${b}_`;
          ...
          if (r) {
            let z = !1;
            v.addEventListener("click", async G => {
              G.preventDefault();
              if (!z) {
                if (v.checked && (z = !0, G = await r(), z = !1, !G)) return;
        ...
        case "slider":
          ...
          n.value = c;
          n.name = b;
          B && (n.style.direction = "rtl");
          n.addEventListener("input", z => {
            A.textContent = l ? l(Number.parseFloat(z.target.value)) : z.target.value;
          });
          const A = document.createElement("div");
          A.className = "gp-ui-slider-preview_";
          A.textContent = l ? l(c) : c;
          A.addEventListener("pointerdown", z => {
            let G = z.clientY;
            const E = I => {
                var H = I.clientY - G;
                const C = 0 > H;
                Math.abs(H) >= GPSettingsManager_.POINTER_SLIDER_PREVIEW_CHANGE_SENSITIVITY && (H = n.valueAsNumber, H += (B ? !C : C) ? D : -D, H = Math.max(Math.min(H, t), w), H = Math.round(1E3 * H) / 1E3, n.value = H, A.textContent = l ? l(H) : H, n.dispatchEvent(new Event("change", {
                  bubbles: !0
                })), G = I.clientY);
              },
              J = I => {
                const H = 0 > I.deltaY;
                let C = n.valueAsNumber;
                C += (B ? !H : H) ? D : -D;
                C = Math.max(Math.min(C, t), w);
            ...
            z.currentTarget.addEventListener("pointermove", E);
            z.currentTarget.addEventListener("pointerup", I => {
              I.currentTarget.removeEventListener("pointermove", E);
            ...
            document.addEventListener("wheel", J, {
              passive: !1
            });
          });
          q.appendChild(n);
        ...
        case "list-numbers":
          ...
          u.forEach(({
            ...
            n.appendChild(H);
            const C = document.createElement("div");
            C.className = "gp-ui-slider-preview_";
            C.textContent = l ? l(c[I]) : c[I];
            C.addEventListener("animationend", N => {
              "list-n-item-highlight_" !== N.animationName && "list-n-item-fadeout_" !== N.animationName || C.classList.remove("up_", "down_");
            });
            C.addEventListener("pointerdown", N => {
              if (0 === N.button) {
                this.resetPreviewAnimation(C);
                C.classList.add("active_");
                var P = N.clientY,
                  R = N.clientY,
                  S = M => {
                    R += M.movementY;
                    var L = R - P;
                    const O = 0 > L;
                    Math.abs(L) >= GPSettingsManager_.POINTER_SLIDER_PREVIEW_CHANGE_SENSITIVITY && (L = c[I], M = E * (M.shiftKey ? 10 : M.ctrlKey ? 100 : 1), L += (J ? !O : O) ? M : -M, L = Math.max(Math.min(L, G), z), L = Math.round(1E3 * L) / 1E3, H.value = L, c[I] = L, C.textContent = l ? l(L) : L, this.updatePreviewAnimation(C, O), H.dispatchEvent(new Event("change", {
                      bubbles: !0
                    })), P = R);
                  },
                  W = M => {
                    0 === M.button && (C.removeEventListener("pointermove", S), document.removeEventListener("wheel", T, {
                      passive: !1
                    }), document.removeEventListener("pointerlockchange", U), document.exitPointerLock(), C.classList.remove("active_"));
                  },
                  T = M => {
                    const L = 0 > M.deltaY;
                    let O = c[I],
                      X = E * (M.shiftKey ? 10 : M.ctrlKey ? 100 : 1);
                    O += (J ? !L : L) ? X : -X;
                    O = Math.max(Math.min(O, G), z);
                    O = Math.round(1E3 * O) / 1E3;
                    H.value = O;
                    c[I] = O;
                    C.textContent = l ? l(O) : O;
                    this.updatePreviewAnimation(C, L);
                  ...
                  U = M => {
                    document.pointerLockElement || (C.removeEventListener("pointermove", S), C.removeEventListener("pointerup", W, {
                      once: !0
                    }), document.removeEventListener("wheel", T, {
                      passive: !1
                    }), document.removeEventListener("pointerlockchange", U), C.classList.remove("active_"));
                  };
                C.setPointerCapture(N.pointerId);
                C.addEventListener("pointermove", S);
                C.addEventListener("pointerup", W, {
                  once: !0
                });
                document.addEventListener("wheel", T, {
                  passive: !1
                });
                document.addEventListener("pointerlockchange", U);
        ...
        case "input":
          ...
          n.classList.toggle("secure-input_", !!u);
          n.spellcheck = !1;
          n.autocomplete = "off";
          n.value = c;
          n.name = b;
          n.addEventListener("keydown", z => {
            switch (z.key) {
              case "Escape":
                z.target.value = z.target.dataset.initialValue;
              case "Enter":
                z.target.blur();
            }
          });
          n.addEventListener("focus", z => {
            z.target.dataset.initialValue = z.target.value;
          });
          n.addEventListener("blur", z => {
            delete z.target.dataset;
          });
        ...
        case "dropdown":
          ...
          (F && f[F] || (y?.length ? y : [c])).forEach((z, G) => {
            const E = document.createElement("option");
            E.value = z;
            E.textContent = l ? l(z, G) : z;
            n.appendChild(E);
          });
          n.value = c;
          q.appendChild(n);
        ...
        case "list-multiple-dynamic":
          u = "list-multiple" === h ? k.items : c;
          n = document.createElement("form");
          n.className = "gp-ui-list-m_";
          n.name = b;
          u.forEach((z, G) => {
            const E = `ctrl-${g.view.dataset.viewIndex}-${b}-${G}_`,
              J = document.createElement("input");
            J.id = E;
            J.type = "checkbox";
            J.value = z;
            J.name = b;
            J.checked = c.includes(z);
            n.appendChild(J);
            z = m && d.l ? d.l[m[G]] : l ? l(z, G) : z;
            G = document.createElement("label");
            G.textContent = z;
            G.title = z;
        ...
        case "list-icons":
          ...
          Object.keys(K).forEach((z, G) => {
            ...
            J.className = "gp-ui-list-i-icon_";
            J.title = z.split("-").map(I => I[0].toUpperCase() + I.slice(1)).join(" ");
            J.style.backgroundImage = `url("${K[z]}")`;
            J.setAttribute("for", G);
            n.appendChild(J);
          });
          q.appendChild(n);
          break;
      ...
      u = ["change", "input"];
      for (let v = 0; v < (p ? 2 : 1); v++) n.addEventListener(u[v], x => {
        x = {
          [x.target.name]: this.parseValue(x.target, h)
        };
        this.dispatchEvent(new CustomEvent("update", {
          detail: {
            module: d,
            storage: e,
            settings: Object.assign(f, x),
            changedSettings: x
          }
        }));
        0 === v && this.updateViews(g.view.moduleName, g.view);
      });
      n.addEventListener("change", v => {
        a[b].dependents && a[b].dependents.forEach(x => {
          this.updateControlState(x, a, f, g);
        });
      });
      return q;
    }
    ...
    parseValue(a, b) {
      switch (a.type) {
        ...
        case "color":
          return "colors" === b ? a.form.value : a.value;
        default:
          return a.value;
      }
    }
  ...
  class GPColorPanel_ extends EventTarget {
    ...
    constructor(a, b, c) {
      ...
      GPUtils_.bindMethods([this.colorsContainerClickHandler, this.colorsContainerDownHandler, this.colorPickerColorChangeHandler, this.colorPickerColor2ChangeHandler, this.colorInputInputHandler, this.colorInputChangeHandler, this.color2InputChangeHandler, this.switchColors, this.wheelHandler], this);
    ...
    init() {
      ...
      this.colorsContainer.addEventListener("contextmenu", this.suppressContextMenu);
    ...
    terminate() {
      ...
      this.colorsContainer.removeEventListener("contextmenu", this.suppressContextMenu);
      ...
      this.colorInput.removeEventListener("pointerdown", this.switchColors);
      this.colorInput.removeEventListener("contextmenu", this.suppressContextMenu);
      ...
      this.color2Input.removeEventListener("contextmenu", this.suppressContextMenu);
    ...
    colorsContainerDownHandler(a) {
      if ((0 === a.button || 2 === a.button) && a.target.classList.contains("color")) {
        ...
        }), a.target.classList.add("sel"), this.palette?.resetPaletteColor(), this.colorPicker?.setColor(b), this.colorInput.value = b, this.dispatchEvent(new CustomEvent("color", {
          detail: {
            color: b
          }
    ...
    initColorInputs() {
      ...
      this.colorInput.addEventListener("contextmenu", this.suppressContextMenu);
      ...
      this.color2Input.addEventListener("contextmenu", this.suppressContextMenu);
  ...
  class GPColorPicker_ extends EventTarget {
    ...
    constructor(a = GPColorPicker_.DEFAULT_WIDTH, b = GPColorPicker_.DEFAULT_HEIGHT, c = GPColorPicker_.DEFAULT_HUE_POSITION) {
      super();
      this.style = document.documentElement.style;
      this.style.setProperty("--cp-hue-margin", `${GPColorPicker_.HUE_MARGIN}px`);
      this.container;
      this.hsv = [0, 0, 0];
      this.width = a;
      this.height = b;
    ...
    paletteMoveHandler(a) {
      if (!this.isHidden) {
        ...
        this.dispatchEvent(new CustomEvent(a.buttons & 2 ? "color2" : "color", {
          detail: {
    ...
    setHSVColor(a, b, c) {
      ...
      this.palettePointerX = Math.max(0, Math.min(this.paletteWidth, b));
  ...
  class GPColorUtils_ {
    ...
    static hexToRgb(a) {
      a = a.replace(/^#?([a-f\d])([a-f\d])([a-f\d])$/i, (b, c, d, e) => c + c + d + d + e + e);
  ...
  class GPColorWheel_ extends EventTarget {
    ...
    buildModeIcon(a) {
      a = GPColorWheel_.MARKERS[a].map(b => `<circle class="cw-ic_" transform="rotate(${30 * b + 180})"/>`);
    ...
    plot(a, b, c, d) {
      ...
      for (c = 0; 4 > c; ++c) {
        ...
        f.addColorStop(1, `rgb(${GPColorUtils_.hsvToRgb((-(d + 90 * c) % 360 + 360) % 360 / 360, 1, 1).join(",")})`);
        a.strokeStyle = f;
        a.beginPath();
        a.moveTo(0, 0);
        a.lineTo(e[c], b[c]);
  ...
  class GPDrawInterface_ extends EventTarget {
    ...
    constructor(a, b, c) {
      ...
      this.settingsTabsData = [[GPDrawInterface_.SETTINGS_TAB.SETTINGS, this.l.SETTINGS_BTN], [GPDrawInterface_.SETTINGS_TAB.SHORTCUTS, this.l.BINDINGS_BTN]];
    ...
    init() {
      ...
      this.toolsContainer.addEventListener("mouseout", this.stopPropagation);
      ...
      this.opacityContainer = this.nativeOpacity.parentElement;
      this.opacityContainer.addEventListener("mouseout", this.stopPropagation);
      ...
      this.thicknessesContainer.addEventListener("mouseout", this.stopPropagation);
      ...
      this.painter.timer && this.painter.prx.isGameTime(GPProxy_.GAME_TIME.HOSTS_DECISION) && this.painter.prx.isHost() && a.appendChild(this.painter.timer.getElement());
      ...
      this.symmetryModes.addEventListener("wheel", c => {
        if ("LABEL" === c.target.tagName) {
          ...
          if (d !== this.painter.getSymmetryMode()) c.target.click();else {
            switch (d) {
      ...
      this.symmetryGuidesCount.addEventListener("wheel", c => {
        c.currentTarget.dataset.count = this.painter.getSymmetryMode() === GPPainter_.SYMMETRY_MODES.RADIAL ? this.painter.changeSymmetryRadialGuidesCount(c.deltaY) : this.painter.changeSymmetryMandalaGuidesCount(c.deltaY);
      });
      ...
      [["mirror", this.painter.getMirrorState()], ["vertical-mirror", this.painter.getVerticalMirrorState()], ["grayscale", this.painter.getGrayscaleState()], ["zoom-in", 1 < this.painter.getZoomScale()], ["zoom-out", 1 > this.painter.getZoomScale()], ["rotation", 0 !== this.painter.getRotation()]].forEach(([c, d]) => {
        const e = `${c}_`,
          f = document.createElement("div");
        f.className = `${e} state_`;
        f.title = this.l.TOOLS_STATES[c];
      ...
      if (!this.settingsPane) {
        ...
        this.settingsPane.addEventListener("pointerdown", this.stopPropagation);
        ...
        this.settingsTabsData.forEach(([d, e]) => {
          ...
          h.className = `${d}-btn_ tab_ gp-ui-btn_`;
    ...
    toolsContainerClickHandler(a) {
      if (a.target.classList.contains("tool")) {
        ...
        if (!a.disabled) {
          if (a.id) this.painter.setTool(a.id);else switch (d) {
            case "hide":
              c = "hideon" === c[1];
    ...
    readyBtnClickHandler(a) {
      a.stopPropagation();
      this.painter.prx.isGameTime(GPProxy_.GAME_TIME.HOSTS_DECISION) && this.painter.prx.isHost() && this.s.readyConfirmation && !this.painter.isReadyWarningShown ? this.showReadyWarning() : (this.painter.timer && this.painter.prx.isGameTime(GPProxy_.GAME_TIME.HOSTS_DECISION) && this.painter.prx.isHost() && this.painter.timer.reset(), a = !this.painter.prx.isReady(), this.updateReadyBtn(a), this.toggleInterface(!a), this.painter.prx.ready());
    }
    showReadyWarning() {
    ...
    renderSettings() {
      ...
      this.currentSettingsTab = GPDrawInterface_.SETTINGS_TAB.SETTINGS;
    }
    ...
    updateThicknessMark(a) {
      if (this.painter.thickness in this.thicknesses) this.thicknessMark.classList.add("hidden", this.painter.thickness in this.thicknesses);else {
        var b = GPDrawInterface_.THICKNESSES.findLastIndex(c => a >= c);
        this.thicknessMark.style.left = `${this.thicknessMarkPositions[b + 1]}px`;
    ...
    calcThicknessMarkPositions(a, b) {
      ...
      return [0, ...c.slice(0, -1).map((h, k) => e + (k + 1) * f + k * g + g / 2), d + 2 * e];
    }
    updateReadyBtn(a) {
      a ?? this.painter.prx.isReady() ? (this.readySvg.innerHTML = GPDrawInterface_.READY_BUTTON_ICON_SVG.EDIT, this.readyLabel.textContent = this.painter.localization.edit) : (this.readySvg.innerHTML = GPDrawInterface_.READY_BUTTON_ICON_SVG.READY, this.readyLabel.textContent = this.painter.localization.ready);
    }
    updateColorPickerPanel(a) {
    ...
    updatePanelsBackground(a, b) {
      ...
      const [f, g, h] = GPColorUtils_.rgbToHsv(c, d, e),
        [k, l, m] = GPColorUtils_.hsvToRgb(Math.max(f - .04246122552758713, 0), Math.max(g - .012523565849717255, 0), Math.max(h - .05882352941176472, 0));
    ...
    updateHeaderColor(a) {
      ...
      k = this.screen.querySelector(":scope .draw").style;
      k.setProperty("--gp-text-stroke", `hsl(${b - 4.27}deg 100% ${Math.max(d - 21.37, 0)}%)`);
      k.setProperty("--gp-book-border-color", `hsl(${b - 10.16}deg ${Math.max(c + 24.18, 0)}% ${Math.max(d - 20, 0)}%)`);
      k.setProperty("--gp-book-bg-color", `hsl(${b - 7.03}deg ${Math.max(c - 4.05, 0)}% ${Math.max(d - 14.12, 0)}%)`);
      k.setProperty("--gp-book-rings-filter", h);
      k.setProperty("--gp-header-color", a);
      k.setProperty("--gp-header-bs-color", `hsl(${b - 5.9}deg ${Math.max(c - 3.28, 0)}% ${Math.max(d + 14.9, 0)}%)`);
      k.setProperty("--gp-header-border-bottom-color", `hsl(${b - 6.19}deg ${Math.max(c + 10.43, 0)}% ${Math.max(d - 25.29, 0)}% / 50%)`);
      k.setProperty("--gp-header-logo-bg-filter", e);
      k.setProperty("--gp-core-bs-color", `hsl(${b - 6.66}deg ${Math.max(c - 4.95, 0)}% ${Math.max(d - 26.28, 0)}%)`);
    }
    ...
    updateInteractionHintsDelay(a) {
      this.style.setProperty("--gp-hints-delay", `${a}s`);
    }
  ...
  class GPExtendedPalette_ extends EventTarget {
    ...
    init() {
      ...
      this.isCustomPalette && (this.palette.draggable = !0, this.palette.addEventListener("dragstart", this.onPaletteDragStart), this.palette.addEventListener("dragover", this.onPaletteDragOver), this.palette.addEventListener("dragenter", this.onPaletteDragEnter), this.palette.addEventListener("dragleave", this.onPaletteDragLeave), this.palette.addEventListener("drop", this.onPaletteDrop));
      ...
      this.palette.addEventListener("pointerdown", a => {
        if (0 === a.button || 2 === a.button) if (this.isCustomPalette) {
          const b = a.target.classList.contains("color_"),
            c = this.colorSet,
            d = b ? this.getColorIndex(a.target) : -1;
          a.shiftKey ? (b ? (c.splice(d, 0, this.getCurrentColor()), c.length = Math.min(c.length, GPExtendedPalette_.MAX_PALETTE_SIZE)) : c.length < GPExtendedPalette_.MAX_PALETTE_SIZE && c.push(this.getCurrentColor()), this.renderPaletteColorSet(), this.saveCustomPalette()) : a.ctrlKey ? (b ? c.splice(d, 1) : c.length && c.pop(), this.renderPaletteColorSet(), this.saveCustomPalette()) : a.altKey ? b && this.renderPaletteColorSet() : b && this.selectPaletteColor(a.target, a.button);
    ...
    setPalette(a) {
      this.palette ? (this.paletteKeys.clear(), this.paletteName = a, this.colorSet = GPExtendedPalette_.PALETTES[this.paletteName], this.isCustomPalette = this.paletteName !== GPExtendedPalette_.PALETTE.DEFAULT, this.paletteDropTarget = null, this.palette.classList.toggle("custom_", this.isCustomPalette), this.renderPaletteColorSet(), this.isCustomPalette ? (this.palette.draggable = !0, this.palette.addEventListener("dragstart", this.onPaletteDragStart), this.palette.addEventListener("dragover", this.onPaletteDragOver), this.palette.addEventListener("dragenter", this.onPaletteDragEnter), this.palette.addEventListener("dragleave", this.onPaletteDragLeave), this.palette.addEventListener("drop", this.onPaletteDrop)) : (this.palette.draggable = !1, this.palette.removeEventListener("dragstart", this.onPaletteDragStart), this.palette.removeEventListener("dragover", this.onPaletteDragOver), this.palette.removeEventListener("dragenter", this.onPaletteDragEnter), this.palette.removeEventListener("dragleave", this.onPaletteDragLeave), this.palette.removeEventListener("drop", this.onPaletteDrop))) : this.init();
    }
    ...
    onPaletteDragStart(a) {
      if (a.shiftKey || a.ctrlKey || a.altKey || !this.colorSet.length) a.altKey && a.target !== a.currentTarget ? (a.dataTransfer.setData(GPExtendedPalette_.TYPE_PALETTE_COLOR_ID, a.target.dataset.id), a.dataTransfer.effectAllowed = "move") : a.preventDefault();else {
        const {
          url: b,
          filename: c
        } = this.buildSerializedPalette();
  ...
  class GPPainter_ extends EventTarget {
    ...
    static SYMMETRY_GUIDES_COUNT_SENSITIVITY = Math.round(5 * GPPainter_.POINTER_RESOLUTION_RATIO_Y);
    ...
    static DEFAULT_SETTINGS = {
      defaultColor: ["#000000", GPPainter_.BACKGROUND_COLOR],
      defaultThickness: 6,
      defaultOpacity: 1,
      minThickness: 1,
      maxThickness: 300,
    ...
    static SETTINGS_UI = {
      ...
      defaultColor: {
        ...
        description: "\u041e\u0441\u043d\u043e\u0432\u043d\u043e\u0439 \u0438 \u0434\u043e\u043f\u043e\u043b\u043d\u0438\u0442\u0435\u043b\u044c\u043d\u044b\u0439 \u0446\u0432\u0435\u0442\u0430 \u043a\u0438\u0441\u0442\u0438 \u043f\u043e \u0443\u043c\u043e\u043b\u0447\u0430\u043d\u0438\u044e (\u0443\u0441\u0442\u0430\u043d\u0430\u0432\u043b\u0438\u0432\u0430\u044e\u0442\u0441\u044f \u0432 \u043d\u0430\u0447\u0430\u043b\u0435 \u043a\u0430\u0436\u0434\u043e\u0433\u043e \u0440\u0430\u0443\u043d\u0434\u0430)"
      },
      defaultThickness: {
        type: "slider",
        args: {
      ...
      opacityStep: {
        ...
        description: "\u0428\u0430\u0433 \u0438\u0437\u043c\u0435\u043d\u0435\u043d\u0438\u044f \u043d\u0430\u0441\u044b\u0449\u0435\u043d\u043d\u043e\u0441\u0442\u0438 \u043a\u0438\u0441\u0442\u0438 \u043f\u0440\u0438 \u0438\u0441\u043f\u043e\u043b\u044c\u0437\u043e\u0432\u0430\u043d\u0438\u0438 \u043a\u043e\u043c\u0431\u0438\u043d\u0430\u0446\u0438\u0439 \u043a\u043b\u0430\u0432\u0438\u0448"
      },
      customOpacity: {
        type: "list-numbers",
        args: {
      ...
      zoomAxis: {
        ...
        description: "\u041f\u043b\u043e\u0441\u043a\u043e\u0441\u0442\u044c \u0440\u0435\u0433\u0443\u043b\u0438\u0440\u043e\u0432\u043a\u0438 \u0443\u043a\u0430\u0437\u0430\u0442\u0435\u043b\u0435\u043c \u043c\u0430\u0441\u0448\u0442\u0430\u0431\u0438\u0440\u043e\u0432\u0430\u043d\u0438\u044f \u0445\u043e\u043b\u0441\u0442\u0430"
      },
      zoomInDirection: {
        type: "dropdown",
        args: {
      ...
      symmetryGuidesRotationMode: {
        ...
        description: "\u0420\u0435\u0436\u0438\u043c \u0432\u0440\u0430\u0449\u0435\u043d\u0438\u044f \u043d\u0430\u043f\u0440\u0430\u0432\u043b\u044f\u044e\u0449\u0438\u0445 \u0441\u0438\u043c\u043c\u0435\u0442\u0440\u0438\u0438: Dragging - \u043f\u0435\u0440\u0435\u0442\u0430\u0441\u043a\u0438\u0432\u0430\u043d\u0438\u0435 \u0443\u043a\u0430\u0437\u0430\u0442\u0435\u043b\u0435\u043c, Snapping - \u043f\u0440\u0438\u0432\u044f\u0437\u043a\u0430 \u043a \u043f\u043e\u0437\u0438\u0446\u0438\u0438 \u0443\u043a\u0430\u0437\u0430\u0442\u0435\u043b\u044f"
      },
      _12: {
        text: "Tools"
      },
    ...
    constructor(a) {
      ...
      this.shortcutsHandler.addEventListener("trigger", ({
        ...
        d.repeat && !this.tools[c].repeat || this.isDrawing && !this.tools[c].activator && !this.tools[c].whileDrawing || this.tools[c].handler.apply(this, [d, ...(this.tools[c].args ?? [])]);
      });
      ...
      this.canvasTypeId = GPUtils_.CANVAS_ID.DEFAULT;
      this.density = GPPainter_.CANVAS_DENSITY;
      this.style = document.documentElement.style;
      ...
      this.isSubscribed = !1;
      this.lastClientY = this.lastClientX = 0;
      this.isDrawing = this.isToolCentering = this.isEraser = this.isGrayscale = this.isVerticalMirror = this.isMirror = !1;
      ...
      this.addEventListener("stroke_started", ({
        ...
        const e = !!d.length,
          f = c[0];
        this.prx.getSendQueueSize() && this.isToolInTime(f) || (this.prx.tool(1, c, e), d.forEach(g => {
          this.prx.tool(1, g, e);
        }));
      });
      ...
      this.addEventListener("stroke_ended", ({
        ...
        const e = !!d.length,
          f = e || this.prx.getSendQueueSize() && this.isToolInTime(c[0]) ? 1 : 3;
        this.prx.tool(f, c, e);
        d.forEach(g => {
          this.prx.tool(f, g, e);
        });
      ...
      Element.prototype.addEventListener = new Proxy(Element.prototype.addEventListener, {
        apply(c, d, e) {
          if (!(d.classList?.contains("drawingContainer") && ["mousedown", "pointerdown", "contextmenu", "touchstart"].includes(e[0]) && b.isPermittedMode())) return c.apply(d, e);
        }
      });
    ...
    terminate() {
      !this.isTerminated && this.container && (clearTimeout(this.strokeTimer), clearTimeout(this.searchTimer), document.removeEventListener("pointermove", this.updateGlobalCoords, !0), this.shortcutsHandler.stop(), window.removeEventListener("blur", this.resetKeys), window.removeEventListener("focus", this.resetKeys), window.removeEventListener("resize", this.resizeWindowHandler), this.clearPointer(), this.clearStroke(), this.clearCanvas(), this.clearBackground(), this.background.classList.add("hidden"), this.lastDraw.classList.add("hidden"), this.colorPicker.hide(), this.iface.terminate(), this.reference?.terminate(), this.container.remove(), this.isTerminated = !0);
    }
    resetTurnSettings() {
      if (this.container) {
        this.color = this.s.defaultColor[0];
        ...
        this.symmetryCoords = [];
        this.symmetryStrokesGroups = {};
        !this.isSubscribed && !this.prx.isSoloGame() && 1 < this.s.symmetryMode && (this.s.symmetryMode = GPPainter_.DEFAULT_SYMMETRY_MODE, this.drawSymmetryGuides());
        ...
        this.isGrayscale = this.isVerticalMirror = this.isMirror = !1;
        this.clearGrid();
        this.resetMirror();
        this.resetZoom();
        this.iface.hideSettings();
    ...
    init() {
      ...
      this.isDisabled = this.isDrawing = this.isToolCentering = this.isEraser = !1;
    ...
    inject() {
      if (this.enabled) {
        var a = this.prx.getCanvasTypeId();
        this.container && this.canvasTypeId === a || this.render(a);
    ...
    isPermittedMode() {
      ...
      for (let b in GPPainter_.DISABLED_MODE_OPTIONS) if (GPPainter_.DISABLED_MODE_OPTIONS[b].some(c => c === a[b])) return !1;
      return !0;
    }
    initColorPicker({
      width: a,
    ...
    strokeDown(a) {
      ...
      if (!this.hasActiveTools()) {
        ...
        this.lineSmoothingCounter = 1;
        this.tool === GPPainter_.TOOL.FILL ? (this.updateStrokeCoords(this.strokeCoords, this.getUnscaledCanvasCoords(a), void 0, b), a = this.buildStrokeData(this.strokeOptions, this.strokeCoords), this.dispatchEvent(new CustomEvent("stroke_started", {
          detail: {
            data: a
          }
    ...
    convertToSymmetryCoords([a, b], c = this.s.symmetryMode) {
      const d = this.isMirror ? this.originalWidth - this.symmetryGuideX : this.symmetryGuideX,
        e = this.isVerticalMirror ? this.originalHeight - this.symmetryGuideY : this.symmetryGuideY,
        f = [];
      switch (c) {
        case GPPainter_.SYMMETRY_MODES.VERTICAL:
          const [l, m] = this.calcSymmetryPoint(a, b, d, e, this.symmetryGuideAngle);
          f.push([l, m]);
          break;
        case GPPainter_.SYMMETRY_MODES.HORIZONTAL:
          const [p, r] = this.calcSymmetryPoint(a, b, d, e, this.symmetryGuideAngle);
          f.push([p, r]);
          break;
        case GPPainter_.SYMMETRY_MODES.QUADRANT:
          c = this.symmetryGuideAngle;
          var g = a - d,
            h = b - e,
            k = g * Math.cos(-c) - h * Math.sin(-c);
          h = -(g * Math.sin(-c) + h * Math.cos(-c));
          g = k * Math.cos(c) - h * Math.sin(c) + d;
          c = k * Math.sin(c) + h * Math.cos(c) + e;
          f.push([g, c], [2 * d - a, 2 * e - b], [2 * d - g, 2 * e - c]);
          break;
        case GPPainter_.SYMMETRY_MODES.RADIAL:
          ...
          a = Math.atan2(k, a);
          k = 2 * Math.PI / c;
          for (g = 1; g < c; g++) h = a + g * k, f.push([Math.round(d + b * Math.cos(h)), Math.round(e + b * Math.sin(h))]);
          break;
        case GPPainter_.SYMMETRY_MODES.MANDALA:
          c = this.s.symmetryMandalaGuidesCount;
          a -= d;
          k = b - e;
          b = Math.sqrt(a * a + k * k);
          a = Math.atan2(k, a);
          k = 2 * Math.PI / c;
          g = a - this.symmetryGuideAngle;
          for (h = 0; h < c; h++) {
            const q = a + h * k,
              n = q - 2 * g;
            f.push([Math.round(d + b * Math.cos(q)), Math.round(e + b * Math.sin(q))], [Math.round(d + b * Math.cos(n)), Math.round(e + b * Math.sin(n))]);
          }
          f.splice(0, 1);
      }
      return f;
    ...
    strokeMove(a) {
      ...
      this.updateStrokeCoords(this.strokeCoords, a);
      this.isSymmetryEnabled && (a = this.convertToSymmetryCoords(a), this.updateSymmetryStrokeCoords(a));
      a = this.buildSymmetryStrokesData(!0);
      this.clearStroke();
      this.drawFunction(this.strokeCtx, [[...this.strokeOptions, ...this.strokeCoords], ...a], this.density, !1);
      this.lineSmoothingCounter++;
    }
    strokeUp(a) {
      clearTimeout(this.strokeTimer);
      this.s.lineSmoothingLevel && this.lineSmoothingCounter % (this.s.lineSmoothingLevel + 1) && this.strokeCoords.push(this.getCanvasCoords(a));
      const b = this.buildStrokeData(this.strokeOptions, this.strokeCoords),
        c = this.buildSymmetryStrokesData(),
        d = this.isToolInTime(this.tool) ? "stroke_ended" : "stroke_started";
    ...
    updateStrokeCoords(a, b, c = 0, d = this.color) {
      switch (this.tool) {
        case GPPainter_.TOOL.BRUSH:
        case GPPainter_.TOOL.ERASER:
        case GPPainter_.TOOL.NOTE:
          this.s.lineSmoothingLevel && (this.tool !== GPPainter_.TOOL.ERASER || this.s.eraserLineSmoothing) && this.lineSmoothingCounter % (this.s.lineSmoothingLevel + 1) || a.push(b);
          break;
        ...
        case GPPainter_.TOOL.FILL:
          a.splice(0, a.length), a.push(...this.getFillData(b[0], b[1], d));
      }
    }
    getStrokeOptions(a = this.tool, b = this.color, c = this.thickness) {
      switch (a) {
        ...
        case GPPainter_.TOOL.ERASER:
          return [a, this.strokeId++, !this.s.doubleEraserThickness || this.isEraser && this.s.disableDoubleEraserThicknessOnShortcut ? c : 2 * c];
        case GPPainter_.TOOL.FILL:
    ...
    buildStrokeData(a, b, c = !1) {
      return [...a, ...(c || !this.isMirror && !this.isVerticalMirror || a[0] === GPPainter_.TOOL.FILL ? b : this.mirrorStrokeCoords(b, this.isMirror, this.isVerticalMirror))];
    }
    lockPointer() {
      this.pointerLock = !0;
    }
    ...
    getPointerCoords(a) {
      ...
      return [(d - c.left) * this.density / b, (a - c.top) * this.density / b];
    }
    renderPointer(a, b, c, d = this.thickness, e, f) {
      this.clearPointer();
      d = "zoom" === this.activator ? 1 : !e && GPPainter_.TOOLS_THICKNESSES[this.tool] || d;
      this.tool !== GPPainter_.TOOL.ERASER || !this.s.doubleEraserThickness || this.isEraser && this.s.disableDoubleEraserThicknessOnShortcut || (d *= 2);
      d *= this.zoomer.getScale();
      c = "thickness" === this.activeTool || "opacity" !== this.activator && "brightness" !== this.activator ? c : this.opacity;
      e = this.pointerCtx;
      var g = Math.min(Math.max(this.zoomer.getScale(), 1), 2);
      e.lineWidth = g;
      e.beginPath();
      ...
      e.stroke();
      e.beginPath();
      e.arc(a, b, (d / 2 + g / 2) * this.density, 0, 2 * Math.PI);
      ...
      d < GPPainter_.POINTER_CROSSHAIR_MIN_THICKNESS && (d = 3 * d * this.density, g = GPPainter_.POINTER_CROSSHAIR_LINE_LENGTH * this.density, e.strokeStyle = GPPainter_.POINTER_OUTLINE_COLOR, e.globalAlpha = this.s.pointerOutlineOpacity, e.beginPath(), e.moveTo(a, b - d), e.lineTo(a, b - d - g), e.moveTo(a + d, b), e.lineTo(a + d + g, b), e.moveTo(a, b + d), e.lineTo(a, b + d + g), e.moveTo(a - d, b), e.lineTo(a - d - g, b), e.stroke(), e.globalAlpha = this.s.pointerOpacity);
      if (f) {
        const [h, k] = f;
        e.beginPath();
        e.arc(h, k, GPPainter_.POINTER_DOT_THICKNESS / 2 * this.density, 0, 2 * Math.PI);
        c && (e.fillStyle = `rgba(${GPColorUtils_.hexToRgb(this.color).join(",")},${c}`, e.fill());
        e.strokeStyle = GPPainter_.POINTER_COLOR;
        e.stroke();
        e.beginPath();
        e.arc(h, k, (GPPainter_.POINTER_DOT_THICKNESS / 2 + 1) * this.density, 0, 2 * Math.PI);
        e.strokeStyle = GPPainter_.POINTER_OUTLINE_COLOR;
        e.globalAlpha = this.s.pointerOutlineOpacity;
        e.stroke();
        e.globalAlpha = this.s.pointerOpacity;
    ...
    resetKeys() {
      this.container && (this.container.classList.remove("dropper_", "hand_", "rotate_", "s-rotate_", "s-adjustable_", "no-pointer_"), this.clearActiveTools(), this.isToolCentering = this.isEraser = !1);
    }
    initPointerTools() {
      for (let a in this.tools) this.tools[a].activator && (this.tools[a].handler = this.initPointerTool(a, this.tools[a].activator));
    }
    initPointerTool(a, b) {
      for (const m in b) {
        const p = b[m];
        "object" !== typeof p ? b[m] = p.bind(this) : GPUtils_.bindDeep(p, this);
      }
      var c = !1,
        d = !1,
        e = null;
      const f = m => {
          ...
          this.container.addEventListener("pointerdown", k);
          c = !0;
          e = m.code;
          this.activator = a;
        },
        g = m => {
          m.code === e && (this.activator === a && (this.activator = null), d || (b.end?.(), this.updatePointer(), document.removeEventListener("keyup", g)), b.keyUp?.(), this.container.removeEventListener("pointerdown", k), c = !1);
        };
      let h = m => {};
      const k = (m, p = !1) => {
          if (!this.isDrawing && this.activator === a && m.buttons & 7) {
            ...
            var r = p ? 0 : m.button;
            b.pointer?.[r]?.hide && this.container.classList.add("no-pointer_");
            b.pointer?.[r]?.pointerDown?.(m);
            b.pointer?.[r]?.pointerMove && (h = b.pointer[r].pointerMove(m));
            this.container.setPointerCapture(m.pointerId);
            this.container.addEventListener("pointermove", h);
        ...
        l = (m, p = !1) => {
          this.container.removeEventListener("pointermove", h);
          h.cancel?.();
          p = p ? 0 : m.button;
    ...
    thicknessPointerMove(a) {
      const [b, c] = this.getPointerCoords(a),
        d = this.s.upThicknessDirection === GPPainter_.DIRECTION.DOWN_RIGHT ? 1 : -1;
      let e = a.clientX,
        f = a.clientY;
      return g => {
        const h = Math.round(Math.max(Math.min(this.thickness + (this.s.thicknessAxis === GPPainter_.AXIS.HORIZONTAL ? g.clientX - e : g.clientY - f) * d, this.s.maxThickness), this.s.minThickness));
        e = g.clientX;
        f = g.clientY;
        h && (this.thickness = h, this.iface.updateThickness(h), this.renderPointer(b, c, void 0, void 0, !0));
      };
    }
    ...
    opacityMove(a) {
      const [b, c] = this.getPointerCoords(a),
        d = this.s.opacityAxis === GPPainter_.AXIS.HORIZONTAL,
        e = `client${d ? "X" : "Y"}`,
        f = this.s.upOpacityDirection === GPPainter_.DIRECTION.UP_LEFT ? -1 : 1,
        g = this.s.opacitySensitivity * (d ? GPPainter_.POINTER_RESOLUTION_RATIO_X : GPPainter_.POINTER_RESOLUTION_RATIO_Y),
    ...
    brightnessMove(a) {
      const [b, c] = this.getPointerCoords(a),
        [d, e, f] = GPColorUtils_.hexToHsv(this.color),
        g = this.s.brightnessAxis === GPPainter_.AXIS.HORIZONTAL,
        h = `client${g ? "X" : "Y"}`,
        k = this.s.upBrightnessDirection === GPPainter_.DIRECTION.UP_LEFT ? -1 : 1,
        l = this.s.brightnessSensitivity * (g ? GPPainter_.POINTER_RESOLUTION_RATIO_X : GPPainter_.POINTER_RESOLUTION_RATIO_Y),
        m = 100 * f / l;
      let p = a[h] * k;
      return r => {
        r = r[h] * k;
        const q = (r + m - p) * l;
        0 > q ? p = r + m : 100 < q && (p = r + m - 100 / l);
    ...
    zoomMove(a) {
      const [b, c] = this.getPointerCoords(a),
        d = this.s.zoomInDirection === GPPainter_.DIRECTION.UP_LEFT ? 1 : -1,
        e = a.clientX,
        f = a.clientY;
      let g = a.clientX,
        h = a.clientY;
      return k => {
        const l = (this.s.zoomAxis === GPPainter_.AXIS.HORIZONTAL ? k.clientX - g : k.clientY - h) * d;
        2 < Math.abs(l) && (this.zoomer.zoom(-Math.sign(l) * this.s.zoomSensitivity, e, f, !1), g = k.clientX, h = k.clientY, this.renderPointer(b, c));
      };
    }
    zoomRightButton(a) {
      this.resetZoom(!0);
    ...
    mouseZoom(a) {
      this.zoomer.zoom(Math.sign(-a.deltaY) * this.s.zoomStep, a.clientX, a.clientY, !0);
      1 >= this.zoomer.getScale() && this.resetZoom(!0);
    ...
    mirrorStrokeCoords(a, b, c) {
      if (a.length) return Array.isArray(a[0]) ? a.map(([d, e]) => [b ? this.width / this.density - d : d, c ? this.height / this.density - e : e]) : a.map(d => d % 2 ? c ? this.height / this.density - d : d : b ? this.width / this.density - d : d);
    }
    resetMirror() {
    ...
    clear() {
      if (!this.isDrawing) {
        var a = this.getStrokeOptions(GPPainter_.TOOL.ERASER, void 0, Math.trunc(Math.hypot(this.originalWidth, this.originalHeight)));
        a = this.buildStrokeData(a, [[this.originalWidth / 2, this.originalHeight / 2]]);
    ...
    circleFilledTool() {
      this.prx.isAnimateMode() || (this.setTool(GPPainter_.TOOL.CIRCLE_FILLED), this.iface.updateToolbar(this.tool), this.updatePointer());
    }
    fillTool() {
    ...
    symmetryPointerDown(a) {
      this.isSymmetryGuidesAdjustable && (this.isSymmetryGuidesMoved = this.isSymmetryGuidesMoving = !0, 2 === a.button && this.container.classList.add("s-rotate_"), this.symmetryGuides.classList.add("no-cursor_"));
    }
    symmetryPointerMove(a) {
      ...
      return f => {
        const [g, h] = this.getMirroredCoords(f, c);
        0 === b ? this.updateSymmetryGuides(Math.max(0, Math.min(this.originalWidth, g)), Math.max(0, Math.min(this.originalHeight, h)), this.symmetryGuideAngle) : 2 === b && (f = this.getPointAngle(this.symmetryGuideX, this.symmetryGuideY, g, h), this.updateSymmetryGuides(this.symmetryGuideX, this.symmetryGuideY, this.s.symmetryGuidesRotationMode === GPPainter_.SYMMETRY_GUIDES_ROTATION_MODE.DRAGGING ? d + (f - e) : f));
      };
    }
    symmetryPointerUp(a) {
    ...
    calcSymmetryPoint(a, b, c, d, e) {
      e = Math.tan(e);
      c = d - e * c;
      d = 1 + e * e;
      return [(a + e * (b - c)) / d * 2 - a, (e * a + e * e * b + c) / d * 2 - b];
    }
    ...
    resetSymmetryGuides() {
      this.updateSymmetryGuides(Math.round(this.originalWidth / 2), Math.round(this.originalHeight / 2), this.getSymmetryModeGuidesAngle(this.s.symmetryMode));
    ...
    drawSymmetryGuides(a = this.symmetryGuideX, b = this.symmetryGuideY, c = this.symmetryGuideAngle) {
      ...
      switch (this.s.symmetryMode) {
        case GPPainter_.SYMMETRY_MODES.VERTICAL:
          ...
          this.drawSymmetryGuide(d, a, b, c);
          break;
        case GPPainter_.SYMMETRY_MODES.HORIZONTAL:
          this.isSymmetryGuidesAdjustable && this.drawSymmetryGuideReticleRing(d, a, b, 7);
          this.drawSymmetryGuide(d, a, b, c);
        ...
        case GPPainter_.SYMMETRY_MODES.QUADRANT:
          this.drawSymmetryGuide(d, a, b, c);
          this.drawSymmetryGuide(d, a, b, c + Math.PI / 2);
          break;
        case GPPainter_.SYMMETRY_MODES.RADIAL:
          var e = this.s.symmetryRadialGuidesCount,
            f = this.symmetryGuideX * this.density,
            g = this.symmetryGuideY * this.density,
            h = 2 * Math.PI / e;
          this.isSymmetryGuidesAdjustable && 2 === e ? this.drawSymmetryGuideReticleRing(d, a, b, 7) : d.arc(a, b, 50 * this.density, 0, 2 * Math.PI);
          for (a = 0; a < e; a++) {
            var k = a * h + c;
            b = f + Math.cos(k) * this.symmetryGuidesRadius;
            k = g + Math.sin(k) * this.symmetryGuidesRadius;
            d.moveTo(f, g);
            d.lineTo(b, k);
          }
          break;
        case GPPainter_.SYMMETRY_MODES.MANDALA:
          for (e = this.s.symmetryMandalaGuidesCount, f = this.symmetryGuideX * this.density, g = this.symmetryGuideY * this.density, h = 2 * Math.PI / e, this.isSymmetryGuidesAdjustable && 2 === e && this.drawSymmetryGuideReticleRing(d, a, b, 7), a = 0; a < e; a++) k = a * h + c, b = f + Math.cos(k) * this.symmetryGuidesRadius, k = g + Math.sin(k) * this.symmetryGuidesRadius, d.moveTo(f, g), d.lineTo(b, k);
      }
      d.stroke();
    }
    drawSymmetryGuide(a, b, c, d) {
      if (1E-10 > Math.abs(Math.cos(d))) a.moveTo(b, 0), a.lineTo(b, a.canvas.height);else {
        d = Math.tan(d);
        b = c - d * b;
        c = a.canvas.width;
        const e = d * c + b;
    ...
    drawSymmetryGuideReticleLine(a, b, c, d, e) {
      e /= 2;
      d += Math.PI / 2;
    ...
    drawSymmetryGuideReticleRing(a, b, c, d) {
      a.arc(b, c, d * this.density, 0, 2 * Math.PI);
    }
    ...
    setSymmetryMode(a) {
      ...
      switch (a) {
        ...
        case GPPainter_.SYMMETRY_MODES.MANDALA:
          ...
          break;
        default:
          this.symmetryGuideX = Math.round(this.originalWidth / 2), this.symmetryGuideY = Math.round(this.originalHeight / 2);
      }
    ...
    changeSymmetryRadialGuidesCount(a) {
      this.s.symmetryRadialGuidesCount = Math.min(GPPainter_.MAX_SYMMETRY_RADIAL_GUIDES, Math.max(GPPainter_.MIN_SYMMETRY_RADIAL_GUIDES, this.s.symmetryRadialGuidesCount + (0 < a ? -1 : 1)));
      this.radiansPerSymmetryRadialGuide = 2 * Math.PI / this.s.symmetryRadialGuidesCount;
      this.drawSymmetryGuides();
    ...
    zoomOut(a) {
      this.isPointerInsidePainter() ? this.zoomer.zoom(-this.s.zoomStep, this.lastClientX, this.lastClientY, !0) : this.zoomer.zoomOut();
    ...
    toggleGrid() {
      ...
      if (0 != a) {
        ...
        for (var h = 0; h < g; h++) {
          var k = h * a;
          const l = (0 == h % 4 ? .5 : .25) * d;
          e.fillRect(f + k, 0, l * d, c);
          e.fillRect(f - k, 0, l * d, c);
        }
        c /= 2;
        f = c / a;
        for (g = 0; g < f; g++) h = g * a, k = (0 == g % 4 ? .5 : .25) * d, e.fillRect(0, c + h, b, k * d), e.fillRect(0, c - h, b, k * d);
        e.globalAlpha = 1;
        this.grid.isConnected || this.pointer.before(this.grid);
      } else this.grid.remove();
    }
    decreaseOpacity(a) {
      this.setOpacity(Math.max(this.s.minOpacity, Math.min(1, Math.round(100 * (+this.opacity - +this.s.opacityStep)) / 100)));
    }
    increaseOpacity(a) {
      this.setOpacity(Math.max(this.s.minOpacity, Math.min(1, Math.round(100 * (+this.opacity + +this.s.opacityStep)) / 100)));
    }
    ...
    disable() {
      this.container.removeEventListener("pointerdown", this.middleButtonDown);
    ...
    getFillData(a, b, c) {
      ...
      if (this.backgroundData) {
        ...
        d = e.getContext("2d");
        d.drawImage(this.background, 0, 0, e.width, e.height);
        d.drawImage(this.lastDraw, 0, 0, e.width, e.height);
        d.drawImage(this.canvas, 0, 0, e.width, e.height);
      } else d = this.canvasCtx;
      return this.getFillRects(d, a, b, c, this.canvas.width, this.canvas.height).map((e, f) => 4 > f % 5 ? Math.round(e / this.density) : e);
    }
    getFillRects(a, b, c, d, e, f) {
      var g = function (t, D, B) {
          var A = D,
            y = B;
          if (m(D, B, t)) {
            for (; m(A + 1, y, t);) A++;
            var F = A;
            do for (A = D - 1, y++; m(A + 1, y, t) && A + 1 <= F;) A++; while (A == F);
            return {
              x: D,
              y: B,
              w: F - D,
        ...
        k = function (t, D, B) {
          ...
          if (m(D, B, t)) {
            for (; m(A, y + 1, t);) y++;
            var F = y;
            do for (y = B - 1, A--; m(A, y + 1, t) && y + 1 <= F;) y++; while (y == F);
            return {
              x: ++A,
              y: B,
              w: D - A,
        ...
        m = function (t, D, B) {
          ...
          t = v.slice(t, t + 4);
          D = Math.abs(B[1] - t[1]);
          var A = Math.abs(B[2] - t[2]),
            y = Math.abs(B[3] - t[3]);
          return 20 > Math.abs(B[0] - t[0]) && 20 > D && 20 > A && 20 > y || 0 == t[3] && 255 == B[0] && 255 == B[1] && 255 == B[2] || 0 == B[3] && 255 == t[0] && 255 == t[1] && 255 == t[2];
        },
        p = function (t) {
          for (var D = t[0], B = t[1], A = D + t[2], y = B + t[3]; D < A; D++) for (var F = B; F < y; F++) r[D][F] = !0;
          return t;
        };
      b = Math.round(b);
      c = Math.round(c);
      ...
      var q = d % 256,
        ...
        u = d = Math.floor(d / 256);
      !function () {
        r = [];
        for (var t = -1; t <= e; t++) r[t] = [];
        r[-1] = [];
        r[e] = [];
        for (t = -1; t <= f; t++) r[-1][t] = 1, r[e][t] = 1;
        for (t = 0; t < e; t++) r[t][-1] = 1, r[t][f] = 1;
      }();
      var v = a.getImageData(0, 0, e, f).data;
      a = 4 * (b + c * e);
      a = [v[a], v[a + 1], v[a + 2], v[a + 3]];
      d = [];
      for (var x = 0; x <= e; x++) d[x] = [];
      if (m(b, c, [u, n, q, 255])) return [];
      for (; m(b - 1, c, a);) b--;
      for (; m(b, c - 1, a);) c--;
      var w;
      q = g(a, b, c);
      ...
      n = q.w;
      u = [].concat(p([q.x, q.y, q.w + 1, q.h + 1]), [0]);
      do {
        b = 0;
        for (2 == c.ref && (b += c.andada); b <= c.h;) -1 != (w = (q = l(a, c.x + c.w + 1, c.y + c.h - b)).h) ? (d[w].push({
          x: q.x,
          y: q.y,
          w: q.w,
          h: q.h,
          ...
          andada: c.h + 1 - b,
          stack: u.length
        }), u.push.apply(u, p([q.x, q.y, q.w + 1, q.h + 1]).concat([c.stack])), w > n && (n = w), b += w) : b++;
        b = 0;
        for (1 == c.ref && (b += c.andada); b <= c.h;) -1 != (w = (q = k(a, c.x - 1, c.y + b)).h) ? (d[w].push({
          x: q.x,
          y: q.y,
          w: q.w,
          h: q.h,
        ...
        b = 0;
        for (4 == c.ref && (b += c.andada); b <= c.w;) -1 != (w = (q = g(a, c.x + b, c.y + c.h + 1)).w) ? (d[w].push({
          x: q.x,
          y: q.y,
          w: q.w,
          h: q.h,
        ...
        b = 0;
        for (3 == c.ref && (b += c.andada); b <= c.w;) -1 != (w = (q = h(a, c.x + c.w - b, c.y - 1)).w) ? (d[w].push({
          x: q.x,
          y: q.y,
          w: q.w,
          h: q.h,
        ...
        for (c = d[n].pop(); null == c && 0 < n;) c = d[--n].pop();
      } while (null != c);
      return u;
    }
    setTimer() {
      this.timer && (this.timer.reset(), this.timer.settings.runAfterTurnStart && this.prx.isGameTime(GPProxy_.GAME_TIME.HOSTS_DECISION) && this.prx.isHost() && this.timer.start());
    }
    stopTimer() {
  ...
  class GPShortcutsHandler_ extends EventTarget {
    ...
    buildShortcutTree(a, b = !1) {
      ...
      for (const d in a) {
        ...
        if (!b || e) for (const f of a[d]) {
          let g = c;
          for (const h of f) g[h] || (g[h] = {}), g = g[h];
          g.name = d;
    ...
    findShortcut(a, b = !1) {
      b = b ? this.shortcutActivatorsTree : this.shortcutTree;
      for (const c of a) {
        if (!b[c]) return null;
        b = b[c];
      }
    ...
    handleDefaultBehavior(a) {
      "Space" !== a.code && "Enter" !== a.code || "BUTTON" !== document.activeElement.tagName || a.preventDefault();
    }
    handleKeyDown(a) {
      if (!this.areKeysDisabled()) if (this.handleDefaultBehavior(a), this.pressedKeys.add(a.code), a.repeat) this.currentShortcut && (this.dispatchShortcut(this.currentShortcut, a), a.preventDefault(), a.stopPropagation());else {
    ...
    matchShortcut(a, b) {
      ...
      c.sort((f, g) => GPShortcutsHandler_.MODIFIER_KEYS_ORDER[f] - GPShortcutsHandler_.MODIFIER_KEYS_ORDER[g]);
  ...
  class GPShortcutsManager_ extends EventTarget {
    ...
    static DEFAULT_SHORTCUTS = {
      ...
      zoom: [["KeyZ"]],
      zoomToPoint: [["KeyS"]],
      zoomIn: [["Equal"], ["ControlLeft", "Equal"]],
      zoomOut: [["Minus"], ["ControlLeft", "Minus"]],
      zoomReset: [["KeyA"], ["ControlLeft", "Digit0"]],
      mirror: [["CapsLock"]],
      ...
      redo: [["ControlLeft", "KeyY"], ["ControlLeft", "ShiftLeft", "KeyZ"]],
      save: [["ControlLeft", "KeyS"]],
      decreaseThickness: [["BracketLeft"]],
      increaseThickness: [["BracketRight"]],
      customThickness1: [["Digit1"]],
      customThickness2: [["Digit2"]],
      customThickness3: [["Digit3"]],
      customThickness4: [["Digit4"]],
      customThickness5: [["Digit5"]],
      thicknessPanelSlot1: [],
      thicknessPanelSlot2: [],
      thicknessPanelSlot3: [],
      thicknessPanelSlot4: [],
      thicknessPanelSlot5: [],
      decreaseOpacity: [["Comma"]],
      increaseOpacity: [["Period"]],
      customOpacity1: [],
      customOpacity2: [],
      customOpacity3: [],
      customOpacity4: [],
    ...
    renderShortcuts(a = this.getShortcuts()) {
      if (this.isRendered) this.shortcutsElem.innerHTML = "";else {
        this.container = document.createElement("div");
        ...
        this.shortcutsElem.classList.add("unlocalized_", !this.l.isLoaded);
      ...
      for (let g in a) {
        ...
        h.className = "shortcuts-container_";
        const k = document.createElement("div");
        k.className = "shortcut-list_";
        h.appendChild(k);
        var f = document.createElement("div");
        f.className = "conflict-warning_";
        k.appendChild(f);
        a[g].length ? a[g].forEach((l, m) => {
          l = this.renderShortcut(g, m, l, k);
          k.appendChild(l);
        }) : (f = this.renderShortcut(g, -1, [], k), f.classList.add("empty_"), k.appendChild(f));
        f = document.createElement("div");
        f.className = "add-btn_ btn_ gp-ui-btn_";
        f.addEventListener("click", l => {
          l = this.renderShortcut(g, -1, [], k);
          k.appendChild(l);
          this.handleViewScrollbar();
          this.handleViewScrollHints();
    ...
    handleViewScrollHints() {
      ...
      this.container.classList.toggle("scroll-hint-bottom_", this.shortcutsElem.offsetHeight + this.shortcutsElem.scrollTop < this.shortcutsElem.scrollHeight);
    }
    ...
    onShortcutSetClick(a) {
      this.activeShortcutElem && this.cancelBinding(this.activeShortcutElem);
      this.keysLocked = !0;
    ...
    stopShortcut(a) {
      this.activeShortcutElem ? this.activeShortcutElem.parentElement.contains(a.target) || this.cancelBinding(this.activeShortcutElem) : document.removeEventListener("pointerdown", this.stopShortcut, !0);
    }
    onKeyDown(a) {
      a.preventDefault();
      a.stopPropagation();
    ...
    handlePressedKey(a) {
      if (0 !== this.pressedKeys.size) {
        ...
        this.pressedKeys.forEach(d => {
          this.isModifierKey(d) ? (b.push(d), c = []) : c = [d];
        });
    ...
    updateShortcutPreview(a) {
      ...
      if (a.size) {
        ...
        const d = this.findShortcut(a, b.dataset.name),
    ...
    updateConflictWarning(a, b, c, d) {
      ...
      b.after(a);
      a.innerHTML = `<div class="icon_"></div><div class="text_">${d ? `<span>${this.l.BINDINGS_RESERVED_KEYS}: </span>${d.map(e => `<span class="shortcut-name_">${this.formatKey(e)}</span>`).join("")}` : `<span>${this.l.BINDINGS_CONFLICT_WARNING} </span><span class="shortcut-name_">${this.l.TOOLS[c][0]}</span>`}</div>`;
    }
    ...
    formatShortcut(a) {
      ...
      for (let c = 0; c < a.length; c++) if (b.append(this.formatKey(a[c])), c < a.length - 1) {
    ...
    buildKeysMap(a) {
      const b = this.painter.getTools(),
        c = {};
      for (let d in a) b[d] && a[d].forEach(e => {
        const f = e.length;
        c[f] = c[f] || [];
        c[f].push({
          name: d,
  ...
  class GPZoomer_ extends EventTarget {
    ...
    constructor({
      ...
      this.rotator.addEventListener("transitionend", e => {
        e.target === e.currentTarget && this.dispatchRotation();
      });
      this.scale = 1;
    ...
    zoom(a, b, c, d = !0) {
      ...
      c = (c - e.top) / this.containerScale;
      if (this.rotation % 360) {
        var f = this.rotation * Math.PI / 180;
        const g = Math.cos(f);
        f = Math.sin(f);
        const h = e.width / 2 / this.containerScale,
          k = e.height / 2 / this.containerScale;
        e = g * (b - h) + f * (c - k) + h;
        b = -f * (b - h) + g * (c - k) + k;
      } else e = b, b = c;
      c = this.scale;
    ...
    zoomToPoint(a, b, c, d = !0) {
      ...
      if (this.rotation % 360) {
        ...
        e = f * h + g * k;
        g = -f * k + g * h;
      } else c = f, b = g, h = e.height / 2 / this.containerScale, e = e.width / 2 / this.containerScale - f, g = h - g;
      h = this.scale;
      this.scale = Math.min(this.maxScale, Math.max(this.minScale, a));
      a = this.scale / h;
      this.positionX = c - (c - this.positionX) * a;
      this.positionY = b - (b - this.positionY) * a;
      this.positionX += e;
      this.positionY += g;
      this.updateTransform(d);
    ...
    pan(a, b, c = !1) {
      if (this.rotation % 360) {
        ...
        a = this.positionX + (a * g + b * h) / this.containerScale;
        b = this.positionY + d;
    ...
    getCoords(a) {
      ...
      return [Math.round((d - this.positionX) / this.scale), Math.round((e - this.positionY) / this.scale)];
    }
    getMinScale() {
    ...
    applyViewportBounds(a, b) {
      ...
      if (!(this.rotation % 360) || 0 !== c) {
        const e = this.viewport.offsetWidth,
          ...
          h = d.offsetHeight * this.scale,
          k = d.offsetLeft * this.scale;
  ...
  class GPReference_ extends EventTarget {
    ...
    constructor(a) {
      ...
      document.addEventListener("gp:ref.image_error", ({
        ...
        b ? this.downloadImage(b, c, d, e, null, f, g, h) : g || (this.pendingImage = null);
        this.hideProgress();
      });
    ...
    terminate() {
      this.container && (this.pendingSearch = this.pendingImage = null, this.isReRenderNeeded = this.isResizing = !1, this.container.remove(), window.removeEventListener("resize", this.handleResizeWindow), this.disable());
    }
    reset() {
    ...
    renderImages({
      ...
      if (a?.length) {
        ...
        if (b) for (this.results.innerHTML = "", this.results.scrollTop = 0, this.results.dataset.query = c, this.columns = [], b = 0; b < this.columnsCount; b++) {
          const g = document.createElement("div");
          g.className = "column_";
          g.dataset.index = `${b}`;
          this.columns[b] = g;
        ...
        a.forEach(g => {
          ...
          k.dataset.previewURL = g.preview.url;
    ...
    getCroppedImage(a, b, c, d) {
      return new Promise(e => {
        ...
        f && g ? (k = document.createElement("canvas"), k.width = f, k.height = g, k.getContext("2d").drawImage(this.image, p, r, f, g, 0, 0, f, g), k.toBlob(e)) : e(null);
      });
    }
    onImageMove(a) {
      let b = a.clientX,
      ...
      this.frames.forEach(k => {
        if (k !== a && !k.isSeparated && !k.isChanged) {
          const l = [k.offsetY, k.offsetY + k.height],
            m = f.get(k.offsetX);
          m ? m.push(l) : f.set(k.offsetX, [[b, b], l]);
          k.offsetX > h && (h = k.offsetX);
        }
      });
      for (let [k, l] of f) for (l.sort((m, p) => m[0] - p[0]), c = 0; c < l.length; c++) if ([, e] = l[c], [g] = c !== l.length - 1 ? l[c + 1] : [document.documentElement.clientHeight, document.documentElement.clientHeight], g - (e + d) >= a.height) return [k, e + d];
  ...
  class GPReferenceFrame_ {
    ...
    moveHandler(a) {
      if (0 === a.button) {
        var b = this.frame.dataset.anchor,
          ...
          k = (l => {
            const [m, p, r] = this.getStickyCoords(l, l.pageX - e, l.pageY - f, g, h);
            this.setPosition(m, p, g, h, r);
          }).bind(this);
        ...
        this.frame.addEventListener("pointerup", () => {
          ...
          this.fm.isSingleFrame() && (this.fm.x = this.offsetX, this.fm.y = this.offsetY, this.dispatchPos(this.offsetX, this.offsetY), this.staticAnchor || this.frame.dataset.anchor === b || (this.fm.anchor = this.frame.dataset.anchor, this.dispatchAnchor(this.frame.dataset.anchor)));
        }, {
          once: !0
        });
    ...
    getStickyCoords(a, b, c, d, e) {
      ...
      this.fm.getFrames().forEach(q => {
        if (q !== this) {
          ...
          var u = Math.abs(n.y - (c + e));
          u < f && (void 0 === k || u < k) && n.x + n.width >= b && n.x <= b + d && (!r || r[0] === q.frame.dataset.anchor[0]) && (k = u, c = n.y - e, m = r = q.frame.dataset.anchor);
          u = Math.abs(n.y + n.height - c);
          u < f && (void 0 === l || u < l) && n.x + n.width >= b && n.x <= b + d && (!r || r[0] === q.frame.dataset.anchor[0]) && (l = u, c = n.y + n.height, m = r = q.frame.dataset.anchor);
          u = Math.abs(n.x - (b + d));
          u < f && (void 0 === g || u < g) && n.y + n.height >= c && n.y <= c + e && (!p || p[1] === q.frame.dataset.anchor[1]) && (g = u, b = n.x - d, m = p = q.frame.dataset.anchor);
          u = Math.abs(n.x + n.width - b);
          u < f && (void 0 === h || u < h) && n.y + n.height >= c && n.y <= c + e && (!p || p[1] === q.frame.dataset.anchor[1]) && (h = u, b = n.x + n.width, m = p = q.frame.dataset.anchor);
        }
      });
      return [b, c, m];
    }
    ...
    setPosition(a, b, c, d, e) {
      ...
      var h;
      if (this.fm.staticAnchor) var k = h = GPReferenceFrameManager_.MIN_FRAME_DISPLAY_AREA_SIZE;else h = c, k = d;
      ...
      this.frame.style.inset = "unset";
      a[0] === GPReferenceFrame_.ANCHOR.TOP ? (this.offsetY = Math.round(k), this.frame.style.top = `${this.offsetY}px`) : (this.offsetY = Math.round(g - (k + d)), this.frame.style.bottom = `${this.offsetY}px`);
      a[1] === GPReferenceFrame_.ANCHOR.LEFT ? (this.offsetX = Math.round(h), this.frame.style.left = `${this.offsetX}px`) : (this.offsetX = Math.round(f - (h + c)), this.frame.style.right = `${this.offsetX}px`);
    }
    getCoords(a) {
    ...
    load(a) {
      ...
      this.image.onload = c => {
        ...
        if (this.isSeparated) this.offsetX = this.fm.x, this.offsetY = this.fm.y, this.frame.dataset.anchor = this.fm.anchor;else {
  ...
  class GPPlayersManager_ extends EventTarget {
    ...
    handleAlbumClick(a) {
      if ("CANVAS" === a.target.tagName && !a.target.parentElement.classList.contains("animation")) {
  ...
  class GPTimelapsePlayer_ extends EventTarget {
    ...
    constructor({
      ...
      this.canvasTypeId = c;
      this.canvasType = GPUtils_.getCanvasTypeById(c);
      const [p, r] = GPUtils_.getCanvasDimensionsById(c);
      this.density = GPTimelapsePlayer_.CANVAS_DENSITY;
      this.originalWidth = p;
      this.originalHeight = r;
      this.width = this.originalWidth * this.density;
      this.height = this.originalHeight * this.density;
      this.originalData = a.slice();
      const [q, n, u] = this.handleDelimiters(a, b);
      this.data = q;
      this.background = n;
      this.auth = d;
      this.author = e;
      ...
      this.defaultDisplayModeDensity = this.getModeDensity(l);
      this.cache;
      this.canvas;
      this.ctx;
      this.isPlaying = !1;
      ...
      GPUtils_.bindMethods([this.progressBarPointerMoveHandler, this.resizeHandler, this.toggleFullscreen, this.playPause, this.drawing, this.expand, this.terminate, this.toggleContextLayout, this.saveImage, this.speedViewPointerMoveHandler], this);
      window.addEventListener("resize", this.resizeHandler);
    ...
    isMStroke(a) {
      return !(1 === a[0] && a[3][0] > this.originalWidth && a[3][1] > this.originalHeight) || a[3][0] - this.originalWidth - GPTimelapsePlayer_.M_STROKE_OFFSET || a.at(-1)[1] - this.originalHeight - GPTimelapsePlayer_.M_STROKE_OFFSET ? !1 : !0;
    }
    render() {
      this.container = document.createElement("div");
    }
    ...
    resizeHandler() {
      ...
      this.pause();
      if (document.fullscreenElement === this.player) this.setSize(document.documentElement.clientWidth, document.documentElement.clientHeight);else {
    ...
    drawing() {
      if (this.stroke) this.drawingStroke(this.stroke);else if (this.pos < this.data.length) {
    ...
    speedViewPointerMoveHandler(a) {
      ...
      a = Math.round(Math.max(0, Math.min(GPTimelapsePlayer_.SPEED_SLIDER_WIDTH - GPTimelapsePlayer_.SPEED_SLIDER_THUMB_WIDTH, a - this.speedInput.currentX)) / ((GPTimelapsePlayer_.SPEED_SLIDER_WIDTH - GPTimelapsePlayer_.SPEED_SLIDER_THUMB_WIDTH) / (GPTimelapsePlayer_.SPEEDS.length - 1)));
    ...
    getCoords(a, b) {
      ...
      const [c, d] = a.touches ? [a.touches[0].clientX, a.touches[0].clientY] : [a.clientX, a.clientY],
        [e, f] = [c - b.left, d - b.top];
    ...
    draw(a, b, c, d, e = !0) {
      if (b) {
        ...
        if (GPTimelapsePlayer_.CACHE_ENABLED && e && b.length > f) {
          if (e = this.getCache(), !this.cache[this.density] && b.at(-1).index + 1 < f) this.drawFunction(a, b, c, d);else {
    ...
    initCacheWorker() {
      ...
      const {
        ...
        {
          density: g
        } = this.getDensityByDimensions(window.screen.width, window.screen.height),
        h = [...new Set([c, f, g])];
      this.worker || (c = URL.createObjectURL(new Blob([this.getFuncBody(this.cacheWorkerBody)])), this.worker = new Worker(c), URL.revokeObjectURL(c), this.worker.addEventListener("message", ({
        ...
        }) : "cache-updated" === k.type && (this.cache = k.cache, this.dispatchEvent(new CustomEvent("cache_updated", {
          detail: {
            density: k.density
          }
        })));
    ...
    cacheWorkerBody() {
      function a(f) {
        ...
        if (!this.cache[f]) {
          ...
          let p;
          self.background && (h.fillStyle = d, h.fillRect(0, 0, g.width, g.height), self.draw(h, this.background, f, !1), p = h.getImageData(0, 0, g.width, g.height), h.clearRect(0, 0, g.width, g.height));
          for (let r = 0; r < m; r++) {
            var k = r * l;
  ...
  class GPTimer_ extends EventTarget {
    ...
    onPointerDown(a) {
      if (0 === a.button && !a.ctrlKey) this.playPause();else if (2 === a.button) this.revertTime();else if (1 === a.button || 0 === a.button && a.ctrlKey) this.state === GPTimer_.STARTED ? this.reset() : this.toggleSetting();
    }
    onWheel(a) {
      var b = 0 > a.deltaY;
      switch (this.state) {
    ...
    revertTime(a) {
      this.state === GPTimer_.STARTED && (void 0 === a && (this.s.reverted = a = !this.s.reverted), this.container.classList.remove("last-minute_"), a = this.formatTime(a ? Math.max(this.value - this.elapsed, 0) : Math.min(this.elapsed, this.value)), this.renderTime(a));
    }
    renderTime({
      h: a,
      m: b,
    ...
    updateSettings({
      ...
      "soundAlertVolume" in a && (this.alert.volume = Number.parseFloat(a.soundAlertVolume), 1 === Object.keys(a).length && this.runAlert(.3));
  ...
  class GPKnownUsersDB_ extends EventTarget {
    ...
    async addUsers(a) {
      if (a?.length) for (let b of a) await this.addUser(b);
    }
    addUser(a, b) {
      return this.db ? new Promise(async c => {
        var d = {
      ...
      if (this.db) {
        ...
        if (!(0 >= a)) {
          ...
          b.index("last_seen").openKeyCursor(null, "next").addEventListener("success", c => {
            (c = c.target.result) && a && (b.delete(c.primaryKey), a--, c.continue());
          });
        }
      }
    }
  ...
  class GPModeration_ extends EventTarget {
    ...
    static NEW_ACCOUNT_PERIODS = ["3600000", "86400000", "604800000", "2678400000"];
    ...
    static newAccountPeriodFormatter(a, b) {
    ...
    constructor(a) {
      ...
      this.preview = new GPPreview_(this.prx, this.s.previewEnabled, this.s.previewPanelPos, this.s.previewPanelWidth, this.l);
      ...
      a.setProperty("--mod-item-new-account", `'${this.l.ITEM_NEW_ACCOUNT}'`);
      ...
      this.prx.addEventListener("data", ({
        ...
        if (this.prx.isAuthorized || this.prx.isTwitchOAuth) b === GPProxy_.EVENT.GAME_STARTED ? this.prx.isAuthorized && this.parseUnknownUsers() : b === GPProxy_.EVENT.TURNS_ENDED ? this.prx.isHost() && document.addEventListener("gp:screen_changed", ({
          ...
          once: !0
        }) : b === GPProxy_.EVENT.GALLERY_SETTINGS && (this.prx.addWSMsgMiddleware([GPProxy_.EVENT.ALBUM_ITEM, GPProxy_.EVENT.ALBUM_INFO], this.middleware), this.isMiddlewareActive = !0, this.init()), this.isMiddlewareActive && [GPProxy_.EVENT.NEW_GAME, GPProxy_.EVENT.ALBUM_SCORE, GPProxy_.EVENT.GAME_QUIT, GPProxy_.EVENT.GAME_STARTED].includes(b) && (this.prx.removeWSMsgMiddleware(this.middleware), this.isMiddlewareActive = !1, this.terminate());
      });
      this.prx.addEventListener("rejoin", ({
        detail: {
          stage: b
        ...
        switch (b) {
          case GPProxy_.STAGE.GAME:
            ...
            break;
          case GPProxy_.STAGE.ALBUM:
            if (this.prx.isAuthorized || this.prx.isTwitchOAuth) this.init(), this.prx.addWSMsgMiddleware([GPProxy_.EVENT.ALBUM_ITEM, GPProxy_.EVENT.ALBUM_INFO], this.middleware), this.isMiddlewareActive = !0;
      ...
      this.prx.isTwitchAuth() && (this.s.userBansSyncWithTwitch && this.checkTwitchTokenStatus(), (this.s.twitchFollowerBadges || this.s.twitchSubscriberBadges) && this.checkTwitchToken2Status());
    }
    getElement() {
      return this.container;
    }
    ...
    renderBanlist() {
      ...
      this.banlist.size ? this.banlist.forEach(d => {
        ...
        if (this.prx.isTwitchAuth() && this.s.disableLocalizedTwitchUsernames) {
          var g = this.localizedLogins.get(this.prx.getUsers().find(k => this.prx.formatNickname(k.nick) === d)?.nick);
          g && (e = g, f = !0);
        }
    ...
    renderUser(a) {
      ...
      })) : this.prx.isTwitchOAuth && a.nick === this.prx.getNickname() && this.av.requestAvatarURL(this.prx.twitchOAuthLogin, g => {
        d.style.backgroundImage = `url(${g})`;
      });
      let e = a.nick;
      c = !1;
      ...
      a.you || this.prx.isTwitchOAuth || this.usersDB.hasUser(a.nick).then(g => {
        g || (b.classList.toggle("unknown_", !g), f.dataset.sinceCreation = e, this.isNewAccount(a.nick) && (b.classList.add("new_"), b.dataset.created = this.getDateCreated(a.nick), b.addEventListener("pointerenter", h => {
    ...
    getSinceCreationTime(a) {
      ...
      switch (this.l.lang) {
        case "ru":
          const f = c % 100,
            g = f % 10;
          if (10 <= f && 20 > f) e = 2;else if (1 === g) e = 0;else if (1 < g && 5 > g) e = 1;else if (!g || 4 < g) e = 2;
          return `${c} ${a[d][e]}`;
        default:
    ...
    clearPlayers(a) {
      this.prx.isHost() && (this.s.autoKick || this.s.autoKickNewAccounts) && (a = a || this.prx.getUsers(), this.prx.isGameStage(GPProxy_.GAME_STAGE.LOBBY) && a.forEach(b => {
    ...
    usersClickHandler(a) {
      a.preventDefault();
      if (this.prx.isAuthorized || this.prx.isTwitchOAuth) if (a = a.target.closest(".ul-user_"), !a?.classList.contains("you_")) {
    ...
    banlistClickHandler(a) {
      ...
      if (b) if (b = b.dataset.username, a.target.matches(".username_")) navigator.clipboard.writeText(a.target.dataset.login ?? b);else if (a.target.matches(".remove-btn_")) {
    ...
    findNickname(a) {
      return this.prx.getUsers().find(b => a === this.prx.formatNickname(b.nick))?.nick;
    }
    middleware([a, b]) {
      ...
      if (a[1] === GPProxy_.EVENT.ALBUM_ITEM && a[2].active) {
        if (this.s.safeMode && a[2].type === GPProxy_.ALBUM_ITEM_TYPE.TEXT && (a[3] = null, b = !0), this.isUserBanned(a[2].user)) if (a[2].data?.length) switch (a[2].type) {
          case GPProxy_.ALBUM_ITEM_TYPE.TEXT:
            this.s.censorSentences && (a[3] = null, b = !0);
        } else this.prx.isGameOrder(GPProxy_.GAME_ORDER.DRAWINGS_WITH_A_BACKGROUND) && this.s.censorDrawings && a[2].bg && (a[2].data = this.prx.getAlbum().background, a[2].bg = !1, this.isAlbumBgHidden = b = !0);
    ...
    async albumItemClickHandler(a) {
      if (0 === a.button) {
        ...
        if (b && !this.isAIImageItem(b)) {
          ...
          if (this.s.copyUsernamesWithClick && this.isAlbumUsername(a.target)) navigator.clipboard.writeText(a.target.dataset.login ?? d);else {
            var e,
              f = !1;
            if (this.isAlbumAvatar(a.target)) this.prx.isYou(d) || this.isAnimationOrMultiPieceItem(b) || (e = this.toggleUserBan(d)) && this.s.sendBanReports && this.reportSender.send({
              ...
              this.shouldBeBannedItem(b, d) || this.isPreviewedItem(b) || b.dataset.cr && b.dataset.cr !== GPModeration_.CENSORING_REASON.BY_USER ? (this.toggleAlbumItemPreview(b), f = !0, this.preview.isEnabled() && (this.preview.clear(), b.classList.remove("previewed_"))) : this.censorItems.has(c) ? this.censorItems.delete(c) : this.censorItems.set(c, b.dataset.type);
            }
            void 0 !== e || this.prx.isGameOrder(GPProxy_.GAME_ORDER.DRAWINGS_WITH_A_BACKGROUND_NO_PREVIEW) && 0 === this.getItemId(b) ? this.handleItems(f ? b : null) : (this.handleAlbumItem(b), this.prx.isAnimateMode() && !this.isAnimationItem(b) && (a = this.getAnimationItem()) && (this.toggleAlbumItemPreview(a, !1), this.handleAlbumItem(a)), this.prx.isMultiPieceMode() && !this.isMultiPieceItem(b) && (b = this.getMultiPieceItem()) && (this.toggleAlbumItemPreview(b, !1), this.handleAlbumItem(b)));
          }
        }
      }
    }
    ...
    shouldByCensoredByBackground(a) {
      return (this.prx.isGameOrder(GPProxy_.GAME_ORDER.DRAWINGS_WITH_A_BACKGROUND_NO_PREVIEW) || this.prx.isGameOrder(GPProxy_.GAME_ORDER.DRAWINGS_WITH_A_BACKGROUND) && !this.isAlbumBgHidden) && 0 < this.getItemId(a) ? (a = this.getNickname(this.albumItems[0]), this.shouldBeBannedItem(this.albumItems[0], a) || this.isCensoredItem(this.albumItems[0])) : !1;
    }
    ...
    shouldBeCensoredByNewAccount(a, b) {
      a = this.isImageItem(a);
      return !this.prx.isYou(b) && this.s.censorNewAccounts && this.unknownUsers.has(b) && (this.s.censorDrawings && a || this.s.censorSentences && !a) && this.isNewAccount(b) ? !0 : !1;
    }
    getItemId(a) {
      return +a.dataset.id;
    }
    ...
    toggleUserBan(a) {
      ...
      c ? (this.banlist.add(b), this.prx.isTwitchOAuth || (this.usersDB.deleteUser(a), this.unknownUsers.add(a))) : this.banlist.delete(b);
    ...
    getContentId(a) {
      return this.prx.isGameStage(GPProxy_.GAME_STAGE.ALBUM) ? `${this.prx.getCurrentAlbumId()}_${this.getItemId(a)}` : `t_${this.prx.getCurrentTurnId()}`;
    }
    isCensoredItem(a, b) {
    ...
    hasCensoredImageItems() {
      return this.albumItems.some(a => this.isCensoredImageItem(a)) ? !0 : this.s.censorDrawings ? this.albumItems.some(a => this.isImageItem(a) && (a.dataset.cr === GPModeration_.CENSORING_REASON.BANNED_PLAYER || a.dataset.cr === GPModeration_.CENSORING_REASON.UNKNOWN_PLAYER || a.dataset.cr === GPModeration_.CENSORING_REASON.NEW_ACCOUNT)) : !1;
    }
    isTextItem(a) {
    ...
    requestTwitchUsersData(a) {
      if (this.prx.isHost() && this.prx.isTwitchAuth()) {
        var b = a.filter(d => !this.prx.isYou(d.nick) && !this.prx.isViewer(d)).map(d => this.prx.formatNickname(d.nick));
    ...
    handleCensorableItem(a) {
      ...
      if (b) if (this.isSuspiciousItem(a) && a.classList.add("suspicious_"), this.shouldBeBannedItem(a, b)) this.censorItem(a, GPModeration_.CENSORING_REASON.BANNED_PLAYER);else {
        if (this.s.safeMode && !this.isAnimationOrMultiPieceItem(a)) {
          if (!this.safeItems.has(this.getContentId(a))) {
            this.censorItem(a, GPModeration_.CENSORING_REASON.SAFE_MODE);
            return;
    ...
    albumItemEnterHandler(a) {
      if (this.preview.isEnabled() && (a = a.currentTarget.closest(".item"), !this.isEmptyItem(a) && !this.isPreviewedItem(a) && a.matches(".banned_, .censored_, .suspicious_"))) {
        ...
        if (a = this.prx.getAlbumItem(b)) b = `${this.prx.getCurrentAlbumId()}_${b}`, this.preview.load(a.data, a.type, b);
      }
    }
    ...
    checkTwitchTokenStatus() {
      this.s.twitchToken && this.twitchAPI.validateToken(this.s.twitchToken).then(a => {
    ...
    handleAlbumLocalizedTwitchUsername(a) {
      if (this.prx.isTwitchAuth()) if (a = this.getNicknameElem(a), this.s.disableLocalizedTwitchUsernames) {
    ...
    handleAlbumHeader(a) {
      if (this.prx.isTwitchAuth() && (a ??= this.contentObserver.getAlbumHeader(), a)) if (this.s.disableLocalizedTwitchUsernames) {
    ...
    handleAlbumFooter(a) {
      if (this.prx.isTwitchAuth() && (a ??= this.contentObserver.getAlbumFooter(), a)) if (this.s.disableLocalizedTwitchUsernames) {
        const b = this.prx.getAlbumAuthor(),
          c = this.localizedLogins.get(b);
    ...
    gallerySettingsObserverCallback(a, b) {
      for (const c of a) if ("childList" === c.type && c.addedNodes[0]?.classList?.contains("fade-enter") && c.addedNodes[0].firstElementChild?.classList?.contains("book")) {
    ...
    updateSettings({
      ...
      "showUI" in a && (this.container.classList.toggle("shown_", a.showUI), this.container.classList.remove("banlist_"), this.renderUsers(), this.activateBtn.classList.toggle("pressed_", a.showUI));
      ...
      "disableLocalizedTwitchUsernames" in a && this.prx.isTwitchAuth() && (this.renderUsers(), this.renderBanlist(), this.prx.isGameStages(GPProxy_.GAME_STAGE.LOBBY, GPProxy_.GAME_STAGE.ALBUM, GPProxy_.GAME_STAGE.GALLERY_SETTINGS, GPProxy_.GAME_STAGE.SCORE) && this.updatePlayerItems(), this.prx.isGameStage(GPProxy_.GAME_STAGE.ALBUM) ? (this.handleAlbumHeader(), this.handleAlbumFooter(), this.handleAlbumUsernames()) : this.prx.isGameStage(GPProxy_.GAME_STAGE.SCORE) && this.updateRankingUsers());
      "newAccountPeriod" in a && this.renderUsers();
      ["censorSentences", "censorNicknames"].some(b => b in a) && this.prx.isGameStage(GPProxy_.GAME_STAGE.ALBUM) && this.handleItems();
  ...
  class GPPreview_ extends EventTarget {
    ...
    toggle(a) {
      ...
      this.enabled ? (this.panel.classList.add("frame-mode_"), this.initWorker()) : (this.clear(), this.window && this.closeWindow(), this.terminateWorker(), this.replaceCanvas());
    ...
    setPanelPosition(a, b, c, d) {
      ...
      b + d / 2 > f / 2 ? (this.panelOffsetY = Math.round(f - (h + d)), this.panel.style.bottom = `${this.panelOffsetY}px`, this.panel.dataset.anchor += "b") : (this.panelOffsetY = Math.round(h), this.panel.style.top = `${this.panelOffsetY}px`, this.panel.dataset.anchor += "t");
    ...
    load(a, b, c) {
      if (a) {
        ...
        if (this.window) this.window.postMessage({
          ...
          data: a,
          itemType: d
        }, GPPreview_.WINDOW_URL);else switch (this.panel.dataset.type = d, b) {
  ...
  class GPTwitchAPI_ extends EventTarget {
    ...
    openAuthWindow(a, b) {
      ...
      a = GPTwitchAPI_.AUTH_URL.replace("{scopes}", encodeURIComponent(a)).replace("%7Btoken-type%7D", encodeURIComponent(b));
